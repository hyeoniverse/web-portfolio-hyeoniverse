  ...
  return (
    <PanelStage>
      ...
      <div className={secu.secGrid}>
        <SortableList count={value.length} onMove={(from, to) => onChange(arrayMove(value, from, to))}>
          {value.map((it, i) => (
            <SortableItem key={i} index={i}>{(row) => (
              <div ref={row.ref} style={row.style} className={`${secu.secItem} ${css.editSecItem}`}>
                ...
                <span className={css.itemTools}>
                  ...
                  <RemoveButton onClick={() => onChange(value.filter((_, x) => x !== i))} />
                </span>
              </div>
            )}</SortableItem>
          ))}
