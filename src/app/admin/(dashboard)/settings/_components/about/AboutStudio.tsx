...
import { parseSqlErd } from "./parseSqlErd";
...
function ErdBlock({ tables, relations, onChange, lang }: {
  ...
  const applySql = () => {
    setErr(null);
    const parsed = parseSqlErd(sql);
    if (!parsed.tables.length) {
      setErr(lang === "ko"
        ? "CREATE TABLE 문을 찾지 못했습니다."
        : "No CREATE TABLE statement found.");
      setResult(null);
      return;
    }
    onChange(parsed.tables, parsed.relations);
    setResult({ tables: parsed.tables.length, relations: parsed.relations.length, skipped: parsed.skipped });
    setImportOpen(false);
    setSql("");
  };
...
>
        ...
        <div className={`${css.erdPanel} ${importOpen ? css.erdPanelOpen : ""}`}>
        {!importOpen ? (
          <div className={css.erdActions}>
            ...
            <div className={css.erdDrop} tabIndex={0} role="button"
              ...
              <span>
                {lang === "ko"
                  ? "여기에 .sql 파일을 놓거나 CREATE TABLE 문을 붙여넣습니다"
                  : "Drop a .sql file here, or paste CREATE TABLE statements"}
              </span>
            </div>
            <Button variant="subtle" size="sm" icon={<FileCode size={14} />} onClick={() => fileRef.current?.click()}>
          ...
          <>
            <p className={css.erdHint}>
              {lang === "ko"
                ? "CREATE TABLE 문을 붙여넣으면 테이블·컬럼·관계를 만듭니다. PRIMARY KEY 와 REFERENCES 로 키와 관계를 인식합니다."
                : "Paste CREATE TABLE statements to generate tables, columns, and relations. PRIMARY KEY and REFERENCES define keys and links."}
            </p>
            <Textarea className={css.erdSql} textareaClassName={css.codeArea} rows={6} autoFocus
              value={sql} onChange={setSql}
              placeholder={"create table posts (\n  id uuid primary key,\n  series_id uuid references series(id),\n  title text\n);"}
              aria-label={lang === "ko" ? "SQL 스키마" : "SQL schema"} />
            <div className={css.erdActions}>
              <Button variant="primary" size="sm" disabled={!sql.trim()} onClick={applySql}>
                {lang === "ko" ? "생성" : "Generate"}
              </Button>
              <Button variant="subtle" size="sm" icon={<FileCode size={14} />} onClick={() => fileRef.current?.click()}>
                {lang === "ko" ? ".sql 파일 업로드" : "Upload .sql file"}
              </Button>
              <Button variant="subtle" size="sm" onClick={() => { setImportOpen(false); setErr(null); }}>
                {lang === "ko" ? "취소" : "Cancel"}
              </Button>
              {err && <span className={css.erdErr}>{err}</span>}
            </div>
