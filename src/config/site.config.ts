...
export const siteConfig = {
  ...
  about: {
    ...
    features: [
      ...
      { icon: "04", title: "Works Horizontal Gallery", description_ko: "작품들을 좌우로 스크롤하며 감상할 수 있는 가로 갤러리입니다. GSAP 기반 양방향 무한 래핑과 한/영 레이아웃 분기를 지원합니다.", description_en: "Browse works in a horizontal gallery with GSAP-powered infinite wrapping in both directions and layout branching for Korean/English.", tech: "GSAP, Infinite Wrapping, i18n Layout, Responsive", image: "/images/screenshots/pc/feat-works.png" },
      { icon: "05", title: "Dark / Light Theme", description_ko: "다크 모드와 라이트 모드를 전환하면 모든 요소가 부드럽게 테마에 맞춰 변합니다. 시스템 설정 감지와 사용자 선택 기억을 동시에 지원합니다.", description_en: "Switch between dark and light modes — every element smoothly transitions. Respects system preferences while remembering user choice.", tech: "CSS Variables, data-theme, prefers-color-scheme, localStorage", image: "/images/screenshots/pc/feat-colors.png" },
      { icon: "06", title: "3D Scroll Torus", description_ko: "스크롤하면 화면 위를 떠다니는 금속 느낌의 3D 도넛 오브젝트입니다. 리사주 곡선 경로를 따라 움직이며 테마에 따라 질감이 바뀝니다.", description_en: "A metallic 3D torus floats along a Lissajous curve path as you scroll, with its texture adapting to the current theme.", tech: "Three.js, React Three Fiber, Lissajous Curve, Environment Map", image: "/images/screenshots/pc/feat-torus.png" },
      { icon: "07", title: "Posts & Series", description_ko: "Supabase 기반 블로그 시스템. Markdown/Rich Text 전환 에디터, 시리즈 발행, 카테고리별 책 모양 카드 탐색, 검색·태그 필터, 커버 이미지(프리셋/Unsplash/AI 생성), 발행 시 Gemini/OpenAI/Claude AI 자동 요약(ko+en)을 지원합니다.", description_en: "A full blog system on Supabase. Switchable Markdown/Rich Text editor, series publishing, book-shaped category browsing, search/tag filtering, cover images (presets/Unsplash/AI generation), and Gemini/OpenAI/Claude auto-summary (ko+en) on publish.", tech: "Supabase, Plate, Series, Canvas API, AI Cover, AI Summary", image: "/images/screenshots/pc/feat-series.png" },
      { icon: "08", title: "Comment & Like System", description_ko: "로그인 없이 쓰는 쓰레드형 댓글입니다. 비밀번호를 함께 받아 수정·삭제 때 대조하고, 관리자 답변 시 이메일로 알립니다. 댓글에는 giscus 식 이모지 반응 8종을, 게시물에는 IP 기준 좋아요를 답니다.", description_en: "Threaded comments that need no sign-in. A password is collected up front and checked on edit or delete, and the admin's reply triggers an email. Comments carry a giscus-style set of 8 emoji reactions; posts carry IP-based likes.", tech: "Supabase, Threaded Replies, Emoji Reactions, bcrypt, Email Notify", image: "/images/screenshots/pc/feat-comment.png" },
    ...
    process: [
      ...
      { step: "02", title_ko: "UI 컴포넌트와 인터랙션 구현", title_en: "UI Components & Interaction", description_ko: "**Hero** 섹션, **Navigation**, **Contact Drawer**, **Loading Screen** 등 주요 UI 컴포넌트를 구현했습니다. 각 컴포넌트의 움직임은 **GSAP**과 **Framer Motion**으로 처리해 Image Velocity, StaggerText, Mouse Parallax, Magnetic Hover 를 만들었습니다. 스크롤 속도와 마우스 움직임에 **스프링 감쇠**로 반응하도록 해서, 값이 목표치로 점차 수렴하며 멈춥니다.", description_en: "Built the core UI components — **Hero** section, **Navigation**, **Contact Drawer**, and **Loading Screen**. Their motion runs on **GSAP** and **Framer Motion**: Image Velocity, StaggerText, Mouse Parallax, and Magnetic Hover. Each responds to scroll speed and pointer movement with **spring damping**, so values converge on their target and settle instead of snapping." },
      { step: "03", title_ko: "다국어 지원 및 반응형 최적화", title_en: "Internationalization & Responsive Design", description_ko: "한/영 **다국어(i18n)** 시스템을 도입하고, 언어별 텍스트 길이 차이로 발생하는 **레이아웃 시프트**를 min-height 예약 방식으로 해결했습니다. 데스크톱·태블릿·모바일 각 환경에 맞는 **반응형 레이아웃**을 구현하고, **clamp() 기반 유동 사이징**을 적용했습니다.", description_en: "Introduced a Korean/English **i18n system** and resolved **layout shifts** from text length differences using reserved min-height. Implemented **responsive layouts** tailored to desktop, tablet, and mobile environments with **clamp()-based fluid sizing**." },
      { step: "04", title_ko: "성능 최적화", title_en: "Performance Optimization", description_ko: "**Lighthouse CLI**로 프로덕션 빌드를 측정하며 2차에 걸쳐 최적화를 진행했습니다. reCAPTCHA를 **invisible 모드 + 지연 로딩**으로 전환하고, 미사용 폰트 4종(12파일)을 제거하여 페이지 용량을 **70% 절감**, 모바일 **Performance 98점**을 달성했습니다.", description_en: "Measured production builds with **Lighthouse CLI** through two rounds of optimization. Switched reCAPTCHA to **invisible mode with lazy loading**, removed 4 unused font families (12 files), reduced page weight by **70%**, and achieved a mobile **Performance score of 98**." },
      { step: "05", title_ko: "문서화 및 프로젝트 회고", title_en: "Documentation & Project Retrospective", description_ko: "기술 선택의 이유, 문제 해결 과정, 아키텍처 구조를 기록하는 **About 페이지**를 구현했습니다. Code Highlights, Design Decisions, Architecture 시각화 등 **15개 패널**을 데스크톱 가로 스크롤과 모바일 세로 레이아웃으로 완성했습니다.", description_en: "Built the **About page** documenting technology choices, problem-solving processes, and architecture structure. Completed **15 panels** — Code Highlights, Design Decisions, Architecture visualization, and more — desktop horizontal scroll and mobile vertical layout." },
