...
describe("jsonServerError", () => {
  ...
  it("프로덕션에서도 원인은 서버 로그에 남는다", () => {
    ...
    expect(logged, "어느 라우트에서 났는지 알 수 있어야 한다").toContain("POST /api/posts");
    expect(logged, "원문이 로그에는 남아야 한다").toContain("posts_slug_key");
  });
  ...
  it("개발 중에는 원문을 그대로 돌려준다 — 네트워크 탭에서 바로 보는 편이 빠르다", async () => {
    ...
    const body = await jsonServerError(dbError, "POST /api/posts").json();
  ...
  it("Error 가 아닌 값도 다룬다", () => {
    ...
    expect(String(spy.mock.calls[0][0])).toContain("supabase 객체 오류");
    expect(String(spy.mock.calls[1][0])).toContain("unhandled");
  });
});
