...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  ...
  },
  {
    ...
    cause: {
      ko: "번역 API 는 DeepL · Google · Gemini · Claude 같은 외부 회사 API 중에서 골라 쓸 수 있도록 만들어 두었습니다 (이런 API 서비스 회사를 \"provider\" 라고 부릅니다). 사용자마다 결제 계정이나 선호 모델이 다를 수 있어, 어느 provider 든 자유롭게 설정해 쓸 수 있게 하려는 의도였습니다.\n\n수정 전에는 사용자가 골라 둔 한 provider 만 호출하는 단순한 구조였습니다. 그러다 다른 기능을 테스트하느라 `.env` 의 API 키 한 줄을 주석 처리해 둔 상태로 번역을 호출하니, 그 한 곳의 인증 실패가 곧장 사용자 화면에 에러 메시지로 노출되었습니다.\n\n이 지점에서 두 가지가 함께 떠올랐습니다.\n\n**① fallback 의 필요성** — 한 사용자가 여러 provider 를 동시에 등록해 둘 수도 있으니, 한 provider 호출이 실패하더라도 다음 provider 가 같은 요청을 처리해 주면 사용자는 실패를 알아채지 못한 채 결과를 받게 됩니다. 그래서 **\"기본 + 백업 우선순위\" 로 여러 provider 를 차례로 시도하는 fallback 패턴** 으로 보완하기로 했습니다.\n\n**② 애초에 실패할 수밖에 없는 상태의 처리** — API 키 자체가 등록되지 않은 provider 라면, 그 기능은 **사용자에게 노출 자체를 하지 않는 것** 이 맞다고 판단했습니다. 사용자 화면에서는 해당 기능을 숨기고, 관리자 화면에만 \"현재 사용 불가\" 안내를 띄워 두는 식으로 정리하기로 했습니다.",
      en: "Translation routes through external providers — DeepL, Google, Gemini, Claude (these external API services are called \"providers\"). The intent was to let each user freely pick whichever provider matched their billing setup or preferred model.\n\nOriginally the flow was simple: call the one provider the user had selected. While testing something else, I commented out an API key in `.env` and ran a translation — and the single-provider auth failure surfaced directly to the user as an error message.\n\nThat moment surfaced two things at once.\n\n**① The need for fallback** — a single user might register multiple providers at once. If one is blocked, another can pick up the same request, so the user never sees a hiccup. I introduced a **\"primary + backup priority\" fallback pattern** that tries providers in order.\n\n**② Handling features that are doomed to fail** — a provider with no API key registered shouldn't be exposed to users at all. The fix: **hide such features from the user-facing UI entirely**, and surface a \"currently unavailable\" notice on the admin side instead.",
    },
    solution: {
