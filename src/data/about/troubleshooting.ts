...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  /* ── Backend / Admin ── */
  {
    ...
    cause: {
      ko: "처음에는 삭제 버튼을 누르면 해당 글이 **DB 에서 곧바로 사라지는** 구조로 만들어 두었습니다. 일반적인 CRUD 패턴 그대로 \"DELETE 요청 = row 삭제\" 로 구현한 셈입니다.\n\n그런데 admin 영역의 CRUD 를 정리하다 보니, **이 구조는 \"한 번의 오조작이 곧 영구 손실\"** 이라는 점이 눈에 띄었습니다. 작성 중이던 글을 실수로 지우면 복구할 수 있는 경로가 전혀 없고, 일괄 삭제 UI 라도 만들면 위험은 더 커집니다.\n\n관리자가 한 명뿐이라 \"실수할 일이 있겠나\" 라고 넘기기 쉬운 부분이지만, 단일 관리자 환경이야말로 **한 번의 실수를 받아 줄 다른 사람도, 검토 단계도 없는 환경** 입니다. 발생 빈도가 낮더라도 한 번 발생했을 때의 비용이 비대칭적으로 크기 때문에, 운영을 시작하기 전에 기본 구조부터 바꿔 두기로 했습니다.",
      en: "Initially, clicking the delete button **wiped the row from the database immediately** — a textbook CRUD implementation where \"DELETE request = row removal\".\n\nWhile cleaning up the admin CRUD, I noticed the structure has a subtle property: **a single misclick equals permanent loss**. There's no recovery path if a draft is accidentally deleted, and the risk only grows once you add bulk-delete UIs.\n\nIt's tempting to dismiss this as \"a single admin won't make mistakes\", but a single-admin environment is precisely **the one with no second person and no review step to catch mistakes**. Even if the failure rate is low, the cost of a single occurrence is disproportionately high — so I decided to redesign the baseline before going to production.",
    },
    solution: {
