...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  ...
  },
  {
    ...
    cause: {
      ko: "번역 API 는 DeepL · Google · Gemini · Claude 같은 외부 회사 API 중에서 골라 쓸 수 있도록 만들어 두었습니다 (이런 API 서비스 회사를 \"provider\" 라고 부릅니다). 사용자마다 결제 계정이나 선호 모델이 다를 수 있어, 어느 provider 든 자유롭게 설정해 쓸 수 있게 하려는 의도였습니다.\n\n수정 전에는 사용자가 골라 둔 한 provider 만 호출하는 단순한 구조였습니다. 그러다 무료 plan 모델로 테스트하던 중 rate limit 에 도달해 호출이 실패했는데, 한 곳만 시도하는 구조라 그 실패가 그대로 에러 메시지로 사용자에게 노출되었습니다.\n\n이 지점에서 **한 사용자가 여러 provider 를 동시에 등록해 둘 수도 있다** 는 점이 눈에 들어왔습니다. 한 곳에서 rate limit 이나 일시 장애가 발생해도 다른 곳이 같은 요청을 받아 줄 수 있으면 사용자는 끊김 없이 결과를 받을 수 있는 만큼, **\"기본 + 백업 우선순위\" 로 여러 provider 를 차례로 시도하는 fallback 패턴** 으로 보완하기로 했습니다.\n\n추가로, API 키가 등록되지 않은 provider 를 선택해 둔 상태에서도 UI 는 해당 기능을 그대로 노출했습니다. \"지금은 사용할 수 없다\" 고 숨기지도 비활성화하지도 않으니, **누가 봐도 실패할 버튼을 사용자가 계속 누를 수 있는** 상태였습니다.",
      en: "Translation routes through external providers — DeepL, Google, Gemini, Claude (these external API services are called \"providers\"). The intent was to let each user freely pick whichever provider matched their billing setup or preferred model.\n\nOriginally the flow was simple: call the one provider the user had selected. Then, while testing on free-tier models, calls started hitting rate limits — and since only one provider was being tried, that single failure surfaced directly to the user as an error.\n\nThat's when it became clear that **a single user might want to register multiple providers at once**. If one hits a rate limit or a momentary outage, another could pick up the same request and the user would never notice — so I introduced a **\"primary + backup priority\" fallback pattern** that tries providers in order.\n\nOn top of that, even when a selected provider had no API key registered, the UI didn't hide or disable the feature. Without a \"currently unavailable\" cue, **users could keep clicking buttons that were guaranteed to fail**.",
    },
    solution: {
