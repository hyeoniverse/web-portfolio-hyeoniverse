...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  /* ── Backend / Admin ── */
  {
    ...
    definition: {
      ko: "관리자가 글을 실수로 지우면 **DB 에서 그대로 사라져 버려서**, 되돌릴 방법이 아예 없었어요.",
    ...
    cause: {
      ko: "처음엔 삭제 버튼을 누르면 그 글이 **DB 에서 곧바로 사라지게** 만들어 뒀어요.\n\n작성 중이던 글을 실수로 지우면 복구할 방법이 없어서, 결국 DB 콘솔에 직접 들어가서 백업이라도 찾아야 했죠.\n\n관리자가 저 혼자뿐이라 \"내가 실수할 일이야 있겠어?\" 하고 안이하게 생각했는데, **막상 써 보니까 버튼을 잘못 누르거나, 여러 글을 한꺼번에 지우다 엉뚱한 글까지 같이 날리는 일이 생각보다 자주** 있더라고요.",
      en: "Initially, clicking the delete button **wiped the row from the database immediately**.\n\nAccidentally deleting a draft left no recovery path — the admin had to log into the DB console directly to look for a backup.\n\nWith only myself as the admin, I figured \"I won't make mistakes\" — but **in real use, miss-clicks and accidental bulk deletes happened more often than I expected**.",
    },
    solution: {
      ko: "삭제 버튼을 눌러도 row 를 진짜로 지우지 않고, **`deleted_at` 이라는 \"삭제한 시각\" 만 기록** 하는 soft delete 방식으로 바꿨습니다. 데이터는 테이블에 그대로 남아 있고, 목록 쿼리에 `WHERE deleted_at IS NULL` 한 줄만 추가하면 사용자 화면에선 안 보이는 거죠.\n\n지운 글은 **휴지통 페이지 (`?trash=true`)** 에서 다시 볼 수 있고, 복원할 땐 `published=false` 로 되돌려 둡니다. 그래야 실수로 복원하다가 **공개 안 한 글이 갑자기 다시 발행되는 사고** 가 안 생기거든요.\n\n진짜로 영원히 지우는 동작 (purge) 은 따로 떼어 별도 API 로 만들어서, **사용자가 한 번 더 명시적으로 확인해야** 실행되도록 했습니다.",
    ...
    keyInsight: {
      ko: "삭제는 **\"없애기\" 가 아니라 \"숨기기\"** 로 시작하는 게 맞습니다.\n\n되돌릴 수 없는 동작은 따로 한 단계 더 두고, **기본 삭제는 언제든 다시 살릴 수 있는 게 안전한 기본값** 이에요.",
  ...
  },
  {
    ...
    definition: {
      ko: "AI 번역과 요약 기능은 DeepL, Gemini 같은 외부 회사 API 를 불러와 처리해요.\n\n테스트 중에 `.env` 의 API 키를 잠깐 주석 처리해 봤더니, 번역이나 요약을 누를 때마다 **\"인증 실패\" 같은 에러 메시지가 사용자 화면에 그대로 떠** 버렸습니다.\n\n이 사이트는 대부분의 API 를 무료 plan 으로 쓰는 터라, **시간당 호출 제한 (rate limit) 에 걸리거나 일시적 장애가 나는 일은 운영하다 보면 충분히 생길 수 있는 일** 이었어요. 테스트로 끝낼 문제가 아니라 진짜로 대비가 필요했죠.",
    ...
    cause: {
      ko: "번역 API 는 DeepL · Google · Gemini · Claude 처럼 여러 곳 중에서 골라 쓸 수 있게 만들어 뒀어요 (이런 API 서비스 회사를 \"provider\" 라고 부릅니다).\n\n근데 \"여러 곳을 지원한다\" 가 \"실제로 그 여러 곳을 다 쓴다\" 와 같은 말은 아니더라고요. 실제 동작은 \"이 기능은 DeepL 한 곳만 호출\" 같은 식이라, **한 번 부를 때 딱 한 곳만 호출** 하고 그게 실패하면 거기서 끝이었어요. 자동으로 다른 곳으로 넘어가는 \"대체 경로 (fallback)\" 가 없었던 거죠.\n\n게다가 API 키가 등록 안 된 provider 를 골라 둔 상태에서도, UI 는 그 기능을 그대로 보여 줬습니다. \"지금은 못 쓴다\" 고 숨기지도, 비활성화하지도 않으니, **누가 봐도 실패하는 버튼을 사용자가 계속 누를 수 있는** 상태가 된 거예요.",
    ...
    solution: {
      ko: "사이트 설정에서 **\"기본 provider + 백업 provider 순서\"** 를 직접 짤 수 있게 바꿨습니다.\n\n예를 들어 기본은 DeepL, 실패 시 백업 순서를 [Gemini → Google → Claude] 로 두는 식이에요. 기본이 실패하면 백업 리스트를 위에서 아래로 차례로 시도하고, 하나라도 성공하면 그 결과를 그대로 반환합니다. 전부 실패해야 그제서야 사용자 화면에 에러를 띄우죠.\n\nAPI 키가 등록 안 된 provider 는 **호출 자체를 시도하지 않고 그냥 건너뜁니다.** 어차피 실패할 게 뻔한 요청을 굳이 보낼 필요는 없으니까요.\n\n그리고 \"배치 번역\" 이라는 케이스도 따로 챙겨 봤습니다. 배치 번역은 한 번의 API 호출에 텍스트 여러 개를 묶어 보내는 방식 (예: 제목 + 본문 + 요약 세 줄을 한 번에 전송) 인데, 가끔 5개를 보냈는데 결과는 4개만 돌아오는 \"부분 실패\" 가 일어납니다. 이걸 모르고 4개로 받아 저장하면 한 줄이 빠진 채로 저장돼 버려요. 그래서 **보낸 개수와 받은 개수를 비교해서 다르면 그 provider 를 실패로 판정** 하고 다음 백업으로 넘어가게 했습니다.",
    ...
    keyInsight: {
      ko: "외부 API 를 쓰는 기능은 설계 단계에서부터 **\"이 API 가 응답을 안 줄 때 사용자에게 뭐가 보일까?\"** 라는 질문에 답해 둬야 합니다.\n\nAPI 가 완전히 죽지 않아도 rate limit 만으로도 충분히 실패하니까요. 그럴 때 **에러를 그대로 보여 줄지 / 기능 자체를 숨길지 / 다른 provider 로 우회할지** 셋 중 하나는 미리 정해 두는 게 안전합니다.",
  ...
  /* ── Frontend / Performance ── */
  {
    ...
    definition: {
      ko: "스팸을 막으려고 넣은 reCAPTCHA 스크립트 (784KB) 가 페이지 로드 직후 바로 다운로드되면서, 초기 렌더링이 어마어마하게 느려졌어요.\n\n수치로 보면 **LCP 17.1초, TTI 18.2초**. LCP 는 \"페이지에서 제일 큰 콘텐츠가 화면에 다 그려지는 데 걸린 시간\", TTI 는 \"사용자가 실제로 클릭이나 스크롤 같은 인터랙션을 할 수 있게 되기까지 걸린 시간\" 인데, \"빠른 페이지\" 의 기준은 LCP 2.5초 이하입니다.\n\n17초는 사용자가 \"이 사이트 안 켜지네\" 하고 떠나 버리고도 남는 시간이죠.",
    ...
    cause: {
      ko: "Google 공식 문서를 그대로 따라서, 앱이 시작하자마자 reCAPTCHA 스크립트를 불러오게 해 뒀어요.\n\n근데 reCAPTCHA 가 실제로 필요한 순간은 **컨택트 폼에서 메시지를 보낼 때 딱 한 번** 입니다. 페이지를 둘러보기만 하는 대부분의 방문자는 끝까지 한 번도 안 쓰는 기능이죠. 그런 스크립트를 모두에게, 매번 784KB 씩 다운로드시키고 있었던 거예요.\n\n그 다운로드와 실행이 **브라우저 메인 스레드를 한참 동안 점유** 하다 보니, 정작 사용자에게 보여 줘야 할 콘텐츠 그리기가 한참 뒤로 밀려난 겁니다.",
    ...
    solution: {
      ko: "스크립트 로드 시점을 **\"앱 시작\" 이 아니라 \"사용자가 페이지에 처음 손을 댄 순간 (클릭, 스크롤, 터치)\"** 으로 늦췄습니다.\n\n사실 컨택트 폼까지 도달하는 사용자는 거의 무조건 그 전에 한 번 이상 인터랙션을 하잖아요. 그러니까 메시지 \"보내기\" 버튼을 누를 즈음이면 스크립트는 이미 백그라운드에서 다 로드된 상태가 됩니다. \"아, 이거 늦게 로드해서 폼이 한 박자 늦게 작동하면 어쩌지?\" 같은 걱정이 없는 거죠.\n\n추가로, Google 서버와의 TCP/TLS 연결만 미리 열어 두는 **`<link rel=\"preconnect\">`** 한 줄을 head 에 넣어 뒀습니다. 그러면 실제로 스크립트를 다운로드할 때 connection 셋업 시간만큼 더 빠르게 시작돼요.",
    ...
    keyInsight: {
      ko: "외부 스크립트를 추가하기 전엔 **\"이게 페이지가 그려지기도 전부터 필요한가?\"** 를 먼저 묻는 습관이 필요합니다.\n\n당장 쓰지도 않을 무거운 파일을 즉시 로드하면, 정작 사용자가 봐야 할 콘텐츠가 몇 초씩 뒤로 밀립니다. \"인터랙션 직전까지는 일단 미뤄 두기 (on-demand)\" 가 거의 항상 더 안전한 기본값이에요.\n\n참고로 reCAPTCHA 는 **컨택트 폼에만** 붙였어요. 댓글은 이미 비밀번호 + IP 해시 이중 인증으로 보호되고 있어서, captcha 까지 더하면 **참여 허들만 올라가고 보안 이득은 별로 없어 보였거든요.**",
  ...
  },
  {
    ...
    definition: {
      ko: "이 사이트는 OS 기본 커서 대신, 마우스를 부드럽게 따라다니는 작은 원 모양의 커스텀 커서를 직접 그려요.\n\n이 커서는 1초에 60번 다시 그려지는 동안 (60fps) 두 가지 일을 동시에 합니다.\n\n**① 위치 부드럽게 따라가기** — 실제 마우스 위치를 향해 한 프레임마다 조금씩 이동. 마우스가 갑자기 멀리 가도 커서가 \"여유 있게 쫓아가는\" 듯한 효과를 만드는 부분이에요.\n\n**② 마우스 아래 요소 판별** — 지금 커서가 클릭할 수 있는 버튼 위인지, 텍스트 입력 영역인지, 비활성 영역인지, 끌 수 있는 영역인지를 확인해서 커서 모양을 거기에 맞게 바꾸는 부분.\n\n둘 다 매 프레임 (16ms 마다) 같은 함수 안에서 같이 돌고 있다 보니, **상대적으로 무거운 ② 가 가벼운 ① 까지 같이 늦춰서 커서가 끊기듯 움직이는 (stutter) 현상** 이 생겼어요.",
    ...
    cause: {
      ko: "② 의 \"마우스 아래 요소 판별\" 을 위해 매 프레임마다 **`elementsFromPoint(x, y)`** 라는 브라우저 API 를 불렀어요. 이 함수는 주어진 좌표 위에 쌓여 있는 모든 HTML 요소를 z-index 역순으로 한 번에 다 돌려줍니다.\n\n근데 복잡한 레이아웃에선 한 좌표 위에 수십~수백 개 요소가 겹쳐 있을 수 있어요. 배경, wrapper, 카드, 텍스트, 인터랙션 영역이 죄다 쌓여 있으니까요. 그걸 매 프레임 다 훑다 보니, **한 프레임 16ms 예산 중 몇 ms 가 ② 에만 그대로 빠져 나갔습니다.**\n\n① 의 위치 계산은 코드 자체는 정말 가벼운데, 같은 루프에 묶여 있어 ② 가 끝날 때까지 기다리느라 함께 늦어진 거죠. 결과적으로 둘 다 동시에 느려진 셈이에요.",
    ...
    solution: {
      ko: "두 작업을 **서로 다른 주기로 떼어 놨습니다.**\n\n**① 위치 따라가기** 는 원래대로 매 프레임 (60fps) 실행 — 부드러움 유지.\n\n**② 요소 판별** 은 60ms 간격으로 한 번만 실행하도록 제한 (이 패턴을 \"throttle\" 이라고 합니다 — 자주 호출되는 함수를 일정 시간에 한 번씩만 실행되게 막는 것). 1초에 60번이 아니라 16번만 돌게 되니, 한 프레임당 비용이 거의 0 에 가까워져요.\n\n② 가 60ms (4 프레임 정도) 늦게 반응하긴 하지만, 사람 눈에 4 프레임 정도의 지연은 거의 \"즉시\" 처럼 느껴집니다. 클릭 가능한 버튼에 마우스를 올렸을 때 모양이 \"바로\" 바뀌는 것처럼 보이거든요. 반대로 ① (위치) 은 한 프레임만 늦어도 stutter 가 바로 눈에 띄니까, **이 쪽만 매 프레임을 지킨 게 핵심** 이에요.\n\n참고로 터치 기기는 애초에 마우스가 없어서, 커스텀 커서 자체를 그냥 꺼 둡니다.",
    ...
    keyInsight: {
      ko: "타이트한 루프 하나에 비용이 다른 두 작업을 같이 묶어 두면, **무거운 쪽이 가벼운 쪽까지 같이 끌어내립니다.**\n\n해결의 출발은 \"이 작업이 정말 매 프레임 필요한 일일까?\" 라고 한 번 의심해 보는 것이에요.\n\n위치처럼 한 프레임만 늦어도 사람 눈에 바로 보이는 일은 **매 프레임 실행 (high-frequency)**, 요소 판별처럼 약간 늦어도 사람이 눈치 못 채는 일은 **빈도를 낮춰서 분리 (throttle / debounce)** — 같은 함수 안의 작업도 비용과 \"체감 속도\" 에 맞춰 따로 운영해야 안 끊겨요.",
  ...
  /* ── CSS / Styling ── */
  {
    ...
    definition: {
      ko: "다크 / 라이트 테마를 부드럽게 전환하려고 \"모든 요소의 배경색·글자색 변화에 transition\" 을 글로벌로 걸었더니, 엉뚱한 부작용이 따라왔어요.\n\n컴포넌트마다 따로 만들어 둔 **\"토글 펼침 / 메뉴 슬라이드 / 모달 페이드인\" 같은 애니메이션이 전부 작동을 안 하는** 거예요. 토글을 눌러도 부드럽게 펼쳐지지 않고 \"툭\" 하고 한 번에 나타나고, 모달도 페이드인 없이 그냥 갑자기 켜지더라고요.",
    ...
    cause: {
      ko: "글로벌 규칙은 `html[data-theme-ready] *` 에 걸려 있어요. 이 선택자의 **specificity (CSS 우선순위 점수)** 는 `(0,1,1)` 인데, 컴포넌트의 단일 클래스 (예: `.modal`) 는 `(0,1,0)` 이라 항상 글로벌에 짓밟히는 구조였습니다.\n\n그리고 `transition` 은 shorthand 속성이라는 점이 결정타였어요. 일반 속성처럼 \"내가 적은 것만 덮어쓰기\" 가 아니라, **그 요소의 \"모든 transition 을 통째로 새로 쓰는\" 동작** 을 합니다. 즉, 글로벌에서 `transition: background-color 0.3s` 한 줄만 적어도 그 요소가 가진 `max-height transition`, `opacity transition`, `transform transition` 같은 다른 transition 들이 한꺼번에 다 날아가 버려요.",
    ...
    solution: {
      ko: "컴포넌트 쪽 transition 이 글로벌 규칙을 이길 수 있도록, **두 클래스를 묶은 복합 선택자 (specificity `(0,2,0)`)** 로 바꿨습니다.\n\n예를 들어 `.modal { transition: opacity 0.3s }` 대신 **`.modalWrap .modal { transition: opacity 0.3s }`** 처럼 적는 거죠. 이러면 specificity 가 `(0,2,0)` 이 되어 글로벌 `(0,1,1)` 보다 한 단계 높아지고, 글로벌 shorthand 를 안전하게 덮어씁니다.\n\n프로젝트의 CLAUDE.md 에도 이 패턴을 \"transition 이 안 먹을 때 가장 먼저 의심해 볼 것\" 으로 적어 뒀어요. 다시는 같은 함정에 안 빠지려고요.",
    ...
    keyInsight: {
      ko: "**CSS 의 `transition` 은 shorthand 속성이라서, 내가 적지 않은 속성의 transition 까지 통째로 날려 버립니다.** 일반 속성처럼 \"덮어쓰기\" 가 아니라 \"교체\" 에 가까워요.\n\n전역에 `*` 선택자로 transition 을 걸 거라면, shorthand 대신 `transition-property` 와 `transition-duration` 을 따로따로 적거나, 영향받을 컴포넌트의 selector specificity 를 미리 한 단계 올려 두는 게 안전합니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "Plate 에디터에서 토글, 콜아웃, 열블록을 만들고 저장한 다음 페이지를 다시 열어 보면, **블록 자체는 남아 있는데 안에 적어 둔 내용만 통째로 비어** 있는 이상한 현상이 있었어요.\n\n예를 들어 \"📝 메모\" 라는 콜아웃 안에 본문 두 줄을 적고 저장했는데, 다시 열어 보면 \"📝 메모\" 라는 박스만 남고 본문은 흔적도 없이 사라져 있었어요. 토글, 콜아웃, 열블록 4가지 종류 모두 똑같이 그랬습니다.",
    ...
    cause: {
      ko: "Plate 는 저장된 HTML 을 다시 에디터 트리 (Slate 노드) 로 복원할 때 \"deserializer\" 라는 함수를 부릅니다. 블록 종류마다 자기 deserializer 의 `parse` 함수를 따로 만들 수 있는데, 그 함수에서 **`children: []` (빈 자식 배열) 를 명시적으로 돌려주는** 코드가 들어가 있었던 게 화근이었어요.\n\nPlate 의 deserializer 는 모드가 둘이에요:\n\n- `parse` 가 `children` 을 **돌려주지 않으면** → HTML 안쪽 자식 노드를 **자동으로 재귀 파싱** (기본 동작)\n- `parse` 가 `children` 을 **명시적으로 돌려주면** → 그 값을 그대로 받아 쓰고, 자동 파싱은 안 함\n\n그러니 `children: []` 라고 적는 순간 Plate 입장에선 \"개발자가 빈 배열을 직접 넘겨 줬으니 자동 파싱 하지 말자\" 라고 해석한 거예요. **HTML 안에 본문이 멀쩡히 들어 있는데도 그걸 무시** 하고 빈 채로 복원했던 거죠.",
    ...
    solution: {
      ko: "각 deserializer 의 `parse` 가 돌려주는 객체에서 **`children: []` 한 줄만 지우면** 끝났어요.\n\n`children` 필드 자체가 없으면 Plate 의 기본 동작이 켜져서, `<div>` 안의 HTML 을 자동으로 재귀 파싱해 Slate 트리로 만들어 줍니다. 4개 플러그인 (토글, 콜아웃, 열 그룹, 열 아이템) 모두에 같은 한 줄 수정만 해 줘서 한꺼번에 해결됐어요.",
    ...
    keyInsight: {
      ko: "라이브러리의 **\"기본 동작 vs 직접 지정한 동작\"** 의 차이를 모르면, 안전해 보이는 코드 한 줄이 자동화 기능을 통째로 꺼 버리는 일이 생기더라고요.\n\nPlate 의 `parse` 에선 `children` 을 **안 적는 것** 이 \"자동 파싱 켜짐\", **빈 배열이라도 적는 것** 이 \"수동 제어 — 자동 파싱 꺼짐\" 이에요. 즉 `[]` 자체가 \"자식이 없다\" 라는 **의도적인 선언** 으로 해석되는 거죠.\n\n\"명시적이 항상 더 안전하다\" 는 직관이 늘 맞지는 않는다는 것, 그리고 라이브러리의 기본값이 어떤 가정 위에서 동작하는지 한 번은 들여다보는 게 좋다는 교훈을 얻었습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "에디터에서 글을 쓰는 동안 일정 주기로 자동저장이 돌아 줘야, 갑자기 브라우저가 꺼지거나 실수로 페이지를 닫아도 작업물이 안 날아갑니다.\n\n처음에는 이 자동저장을 `localStorage` (브라우저가 자체적으로 가진 로컬 저장 공간) 에 그냥 직접 적어 뒀는데, 시간이 지날수록 문제가 하나둘 보이기 시작했어요.\n\n**① 다른 기기에서는 임시 저장이 안 보임** — 집 PC 에서 쓰던 글을 노트북에서 이어 쓰려고 들어가면 아무것도 없어요.\n\n**② 새로고침만 해도 \"저장됨\" 알림이 뜸** — 글 내용은 하나도 안 건드렸는데 시스템은 \"변경됨\" 이라고 판단해서 매번 새 임시본을 만들어 둠.\n\n**③ \"불러올까요?\" 알림이 무한 반복** — 에디터를 다시 열면 \"전에 저장된 임시본 불러올까요?\" 가 뜨는데, \"무시\" 를 눌러도 새로고침하면 같은 내용으로 또 물어봅니다.",
    ...
    cause: {
      ko: "원인이 세 가지가 겹쳐 있었어요.\n\n**① localStorage 자체의 한계** — 말 그대로 \"같은 브라우저, 같은 기기\" 안에서만 접근돼요. 다른 PC 나 모바일에선 아예 안 보이는 게 정상 동작입니다. 임시 저장을 환경 사이로 옮기고 싶으면 결국 서버에 두는 수밖에 없어요.\n\n**② \"변경 없음\" 비교 기준이 처음부터 어긋나 있음** — 코드 안에 \"마지막으로 저장된 내용\" 을 기억하는 변수가 있는데, 그 초기값이 빈 문자열 `\"\"` 이었어요. 그런데 실제 폼 상태를 JSON 으로 변환하면 비어 있어도 `\"{}\"` 가 나오니, 두 값이 다르죠. 그래서 mount 직후부터 시스템은 \"변경된 상태\" 로 판단했고, 새로고침 = 폼 mount 마다 새 임시본이 만들어졌습니다.\n\n**③ 사용자가 무시한 임시본을 어디에도 안 적어 둠** — 사용자가 \"불러오기\" 알림을 무시해도, 그 임시본 자체는 DB 에 그대로 남아 있어요. 다음 진입 때 시스템은 같은 임시본을 또 발견하고는, \"이거 새로 보여 줘야 할 것\" 인지 \"이미 무시한 것\" 인지 알 길이 없으니 또 알림을 띄웠던 거예요.",
    ...
    solution: {
      ko: "①②③ 을 하나씩 해결했어요.\n\n**① 저장소를 서버 DB 로 이전** — `localStorage` 는 완전히 빼 버리고, Supabase 의 `revisions` 테이블 (이전에 저장된 모든 임시본을 row 단위로 모아 두는 \"버전 기록\" 테이블) 을 유일한 저장소로 만들었습니다. 이제 다른 기기에서 로그인만 하면 작성 중이던 글이 그대로 따라옵니다.\n\n**② 비교 기준값을 mount 시점의 실제 값으로 초기화** — \"마지막 저장된 내용\" 변수의 초기값을 빈 문자열이 아니라, **폼이 mount 된 직후의 실제 JSON 값** 으로 두도록 고쳤습니다. 이러면 \"진짜로 바뀐 경우\" 만 자동저장이 발동해요.\n\n**③ 무시된 임시본 따로 기억** — 사용자가 \"무시\" 한 임시본의 식별값을 메모리상 `Set` (중복 없이 모아 두는 컬렉션) 에 같이 저장. 시스템이 같은 임시본을 또 발견해도 Set 안에 있으면 알림을 안 띄웁니다.\n\n**브라우저 닫는 순간의 마지막 저장 보장** — 사용자가 브라우저를 닫거나 다른 페이지로 이동하는 그 짧은 순간에도 마지막 변경분이 안 날아가도록, **`navigator.sendBeacon`** (브라우저가 종료 중일 때도 안전하게 서버로 데이터 전송이 끝나게 해 주는 표준 API) 과 **`fetch({ keepalive: true })`** (페이지가 사라져도 요청만은 끝까지 살려 두는 옵션) 를 같이 씁니다.",
    ...
    keyInsight: {
      ko: "자동저장의 본질은 \"언제 저장할까\" 보다 **\"언제 저장하지 않을까\"** 에 더 가깝습니다.\n\n비교 기준값만 정확히 잡아 두면 ② \"바뀐 거 없는데 저장\" 케이스가 사라지고, ③ \"사용자가 무시한 임시본은 다시 묻지 않게\" 추적까지 더하면 **의미 있는 변경 시점만** 기록에 남아요.\n\n안 그러면 한 시간 작업하고 보면 \"단어 하나 고침\" 짜리 임시본이 수십 개씩 쌓여서, 정작 되돌리고 싶은 그 시점을 찾을 수가 없어집니다. 직관적으로는 \"많이 저장 = 안전\" 같지만, 실제로는 **\"의미 있는 시점만 저장 = 안전\"** 인 거죠.",
  ...
  },
  {
    ...
    definition: {
      ko: "홈 화면 CTA 섹션의 \"Contact / Resume\" 버튼은 호버하면 뒤쪽 3D 커피잔이 흐릿하게 비치는 \"유리창 너머\" 같은 효과 (`backdrop-filter: blur()`) 를 줍니다.\n\n그런데 Chrome 에서만 이 효과가 **아예 안 보였어요.** 그냥 반투명 tint 만 살짝 깔리는 정도였죠. DevTools \"Computed\" 탭에서 `backdrop-filter` 속성은 분명 적용돼 있는데, 실제 blur 가 일어나는 단계 (\"sampling\") 가 동작 안 한 셈이에요.",
    ...
    cause: {
      ko: "홈 진입 애니메이션이 `.home` 래퍼를 `y: '100vh' → 0` 으로 위로 올라오게 하는 **transform 기반** 이었어요.\n\n애니메이션이 끝난 뒤에도 framer-motion 이 `transform: translate3d(0,0,0)` 와 `will-change` hint 를 그대로 남겨 둡니다. 사소해 보여도 이게 결정타였어요. **`.home` 이 자기만의 compositing layer (브라우저가 내부적으로 별도 GPU 텍스처로 분리해 그리는 layer) 로 승격되는 트리거** 가 되거든요.\n\n일단 layer 가 분리되면, 그 안쪽 자식의 `backdrop-filter` 는 그 layer **안에 있는 픽셀만** 샘플링할 수 있어요. layer 바깥쪽 (= 그 아래의 3D 커피 canvas) 은 아예 \"뒤에 아무것도 없는 것\" 으로 취급해 버리니까, blur 가 \"되긴 하는데 비빌 대상이 없어서\" 결과적으로 안 보이는 거였습니다.\n\n추가로 `-webkit-backdrop-filter` 접두사가 환경에 따라 Chrome 의 declaration 파싱을 꼬이게 만드는 경우도 있어서 효과를 더 약하게 만들었어요.",
    ...
    solution: {
      ko: "진입 애니메이션을 **`y` (transform 기반) → `marginTop` (layout 기반)** 으로 갈아 끼웠습니다.\n\nmargin / padding / width 같은 layout 속성은 GPU 가 아니라 CPU 에서 처리되고, compositing layer 를 만들지도 않거든요. 그래서 `.home` 이 다시 \"평범한 layer\" 로 돌아오고, 그 아래 버튼의 `backdrop-filter` 가 layer 경계 너머의 커피 canvas 까지 정상적으로 샘플링하게 됐습니다.\n\n그리고 layer 를 분리하는 다른 트리거 (예: `.home` 의 `border-radius` + `overflow: clip` 조합) 도 같이 정리하고, `-webkit-backdrop-filter` 접두사도 그냥 빼 버렸어요. 요즘 Chrome 은 표준 `backdrop-filter` 만 알아 봐도 충분합니다.",
    ...
    keyInsight: {
      ko: "`backdrop-filter` 는 요소와 **같은 compositing layer 안에 있는 배경만** 샘플링할 수 있습니다.\n\n조상 중 누군가가 `transform`, `will-change: transform`, `filter`, `mask`, `isolation: isolate` 같은 속성으로 layer 를 분리해 버리면, 그 경계 너머의 배경은 그냥 \"없는 셈\" 이 돼 버려요.\n\n버튼 호버 같은 국소적인 blur 가 안 먹으면, filter 자체를 의심하기 전에 **조상 경로 어디에도 layer 승격 속성이 안 끼어 있는지** 먼저 확인해 보는 게 빠릅니다.\n\n그리고 transform 기반 애니메이션은 끝난 뒤에도 compositing hint 를 남겨 두는 경우가 많아요. 한 번만 실행되는 진입 애니메이션이라면, **layout 속성 (margin / padding / width) 으로 대체할 수 있을지** 한 번 고민해 볼 가치가 있습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "글 카드 (PostCard) 를 누르면 카드의 이미지가 상세 페이지의 hero 영역으로 자연스럽게 \"모핑\" 되면서 이동하도록 만들어 둔 효과가 있어요.\n\n그런데 그 전환에서 두 가지 버그가 동시에 일어났습니다.\n\n**① hold 단계에서 무한 정지** — 이미지가 hero 크기로 축소된 뒤에, 오버레이가 안 사라지고 화면 위에 영원히 떠 있는 채로 남았어요.\n\n**② 스켈레톤 깜빡임** — 모핑이 끝난 직후, 그 아래쪽으로 로딩 스켈레톤이 잠깐 노출되는 \"이미지 축소 → 스켈레톤 살짝 보임 → 실제 페이지 등장\" 어색한 시퀀스가 생겼습니다.",
    ...
    cause: {
      ko: "두 원인이 따로따로 있었어요.\n\n**① 무한 hold** — 원래 설계는 `expand → morph(hero 크기) → hold` 까지 자동으로 진행되다가, 새 페이지의 hero `<motion.div>` 에 걸린 `onAnimationStart` 콜백 안에서 `endTransition()` 을 호출해서 오버레이를 닫는 구조였어요.\n\n근데 hero 의 `initial` 과 `animate` 둘 다 `opacity: 1` 로 두었거든요 (전환 중에 hero 가 즉시 보이도록). framer-motion 입장에선 \"시작 값 == 끝 값\" 이라 \"실제로 애니메이션할 게 없는 no-op\" 으로 판정합니다. 그러면 **`onAnimationStart` 가 아예 발화하지 않고**, 그 안의 `endTransition()` 도 안 불려서 phase 가 영원히 hold 에 갇히는 거였어요.\n\n**② 스켈레톤 깜빡임** — morph 가 클릭 후 고정 타이밍 (약 1초) 으로 진행되다 보니, **새 페이지가 다 준비되기도 전에 오버레이가 먼저 작아져 버렸어요**. 그 빈 자리는 Next.js 의 `loading.tsx` (Suspense fallback) 인 스켈레톤이 차지하고 있다 보니, 사용자에게 한 박자 노출돼 버렸습니다.",
    ...
    solution: {
      ko: "전환 상태 머신을 다시 설계해서 두 문제를 한 번에 잡았어요.\n\n**① endTransition 트리거를 다른 신호로 변경** — `onAnimationStart` 의존을 빼고, **새 페이지 컴포넌트의 `useEffect` 에서 mount 시점에 `endTransition()` 을 호출** 하게 바꿨습니다. 이건 \"애니메이션이 진짜 돌아갔는지\" 와는 무관하게 \"새 페이지가 마운트됐다\" 라는 확실한 신호예요.\n\n그리고 무슨 이유로든 그 호출이 안 일어날 경우에 대비해 **`SAFETY_MS=5000` 안전망 타이머** 도 PageTransitionProvider 에 같이 넣어 뒀어요. 5초 안에 신호가 안 오면 그냥 강제로 dismiss.\n\n**② hold 단계 동안 backdrop 을 화면 전체로 유지** — 원래는 backdrop 도 hero 영역 크기에 맞춰 작아졌는데, 이제는 hold 동안 backdrop 이 화면 전체를 그대로 덮고 있어요. 그래서 morph 가 끝난 뒤에도 그 아래의 스켈레톤은 안 보입니다.\n\n빠른 mount (데이터 캐시가 잡힌 경우) 케이스를 위해 `endRequestedRef` flag 도 추가했어요. 새 페이지가 morph 보다 먼저 준비되면 hold 를 건너뛰고 곧바로 done 으로 넘어갑니다.",
    ...
    keyInsight: {
      ko: "두 가지 교훈을 얻었어요.\n\n**① 중요한 상태 전환을 애니메이션 라이프사이클 콜백 하나에만 의존하지 말기.**\n\n`onAnimationStart` / `onAnimationComplete` 같은 콜백은 \"실제로 값이 변하지 않을 때\" 호출이 통째로 누락될 수 있고, 라이브러리 버전이나 렌더링 타이밍에 따라 조용히 실패하기도 합니다. 항상 `useEffect` 기반 fallback 이나 setTimeout 안전망과 같이 설계해야, 콜백이 안 와도 시스템이 안 죽어요.\n\n**② 모핑 전환에선 \"오버레이가 작아지는 순간 그 아래가 노출된다\" 는 시각적 계약을 잊지 말기.**\n\nSuspense fallback (스켈레톤) 이 있는 환경에서 \"이미지가 hero 로 작아지는\" 식의 전환을 만들 때, 새 페이지가 다 준비되기 전에 morph 가 먼저 끝나 버리면 스켈레톤이 그대로 노출됩니다.\n\n해결법은 결국 두 가지뿐이에요. (a) backdrop 으로 morph 가 끝난 뒤에도 화면 전체를 덮어 두거나, (b) 새 페이지가 mount 될 때까지 morph 자체를 미루거나. 둘 다 본질은 **\"오버레이의 시각 상태가 실제 페이지 상태와 동기화되도록 만들기\"** 입니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "`/posts` 페이지의 bento 레이아웃은 비율이 다른 카드 5종을 한 grid 안에 섞어 써요 — wide (가로형), 21:9 banner, 1:1 square, 3:4 portrait, 16:10 standard.\n\n근데 일반 CSS Grid 만 쓰면 한 row 의 높이가 \"그 row 안에서 가장 큰 카드\" 기준으로 늘어나 버려요. 그래서 작은 square 카드 옆에 큰 portrait 카드가 들어가면, **square 아래쪽에 portrait 와의 높이 차이만큼 빈 공간** 이 그대로 남아 버립니다. Pinterest 같은 진짜 masonry 처럼 빈틈 없이 채워지질 않는 거죠.\n\n`grid-auto-flow: dense` (작은 셀이 비어 있는 자리를 backfill 해 주는 옵션) 를 같이 써도 가로 빈자리는 좀 채워지긴 해요. 근데 **세로로 남는 공간** 은 여전히 손도 못 댑니다.",
    ...
    cause: {
      ko: "CSS Grid 의 `grid-template-rows: auto` (혹은 고정 비율 row) 는 한 row 의 높이를 \"그 row 안의 자식 중 가장 큰 것\" 에 맞춰 묶어 둡니다.\n\n그러니까 한 row 에 1:1 square 카드와 3:4 portrait 카드가 같이 들어가면, row 전체가 portrait 의 높이까지 쭉 늘어나면서 square 아래에 그만큼의 dead space 가 생기는 거죠. 결국 row 자체가 \"한 단위 = 카드 한 줄\" 로 묶여 있어서, 다른 카드가 그 미세한 잔여 공간을 비집고 들어갈 길이 없는 구조였던 겁니다.",
    ...
    solution: {
      ko: "결국 **진짜 masonry 를 CSS Grid + JS 하이브리드** 로 직접 구현했어요.\n\n**CSS 쪽** — `grid-auto-rows: 1px` 으로 row 단위를 1px 단위까지 아주 잘게 쪼개 두고, `grid-auto-flow: dense` 와 `gap` 정도만 지정합니다.\n\n**JS 쪽** (useEffect) — 모든 카드의 실제 높이를 `firstElementChild.scrollHeight` 로 측정한 다음, `span = ceil((높이 + gap) / (rowUnit + gap))` 공식으로 각 카드가 차지할 row 수를 계산해서 `style.gridRow = span N` 을 동적으로 붙여 줍니다. row 단위가 1px 이라 카드 높이를 거의 픽셀 단위까지 정확하게 맞출 수 있어요.\n\n폰트나 이미지가 늦게 로드되면 처음 측정한 높이가 어긋날 수 있어서, **`ResizeObserver` (grid 크기 변화 감지) 와 이미지 `onLoad` 두 시점 모두** 에서 다시 측정·재계산합니다.\n\n모바일 (≤ 640px) 에선 화면이 좁아서 variant 조합 자체가 어색해지더라고요. 그래서 모든 카드를 16:10 한 가지 비율로 통일하고, JS 측정도 그쪽에선 아예 끄고 비용을 줄였습니다.",
    ...
    keyInsight: {
      ko: "순수 CSS 만으로 masonry 를 만드는 건 아직 실험적인 단계예요. `grid-template-rows: masonry` 는 Firefox 외엔 지원이 안 돼서 Chrome 에서 그대로 쓸 수 없습니다.\n\n그래서 빈틈 없이 packing 하는 \"사실상의 표준\" 은 **row track 을 1px 같은 작은 단위로 잘게 쪼개고, JS 가 측정한 카드 높이로 span 을 동적으로 부여하는 하이브리드** 입니다.\n\n측정값으로는 `firstElementChild.scrollHeight` 가 제일 정확해요. wrapper 자체의 padding 이 합쳐져 들어가지 않아서, 픽셀 수치가 \"진짜 콘텐츠 높이\" 와 딱 맞아떨어지거든요. 그리고 이미지 `onLoad` 와 `ResizeObserver` 두 시점 모두에서 재측정을 돌려야, 폰트나 이미지가 뒤늦게 로드돼도 어긋난 span 이 그대로 굳지 않습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "관련글 chip 의 순서 변경 (RelationPicker) 과 작품 페이지네이션 정렬 (SortOrderDragList) 두 군데에서, \"드래그해서 순서 바꾸기\" 기능을 만들었어요.\n\n처음엔 HTML5 표준 드래그 앤 드롭 (D&D) 으로 짰는데, 동시에 세 가지 문제가 줄줄이 터졌습니다.\n\n**① 드래그가 안 시작됨** — \"지금 잡고 있는 chip 만 draggable 속성을 켜자\" 라는 패턴 (`draggable={dragId === id}`) 이 React 의 batching 때문에 DOM 반영이 한 박자 늦어요. 사용자가 처음 잡는 순간엔 `draggable=false` 인 채로 `dragstart` 이벤트가 떠 버리고, 브라우저는 그걸 무시해 버립니다.\n\n**② 동일 코드인데 비대칭 동작** — 같은 코드인데 \"뒤에 있는 chip 을 앞으로\" 는 잘 되는데, \"앞에 있는 chip 을 뒤로\" 만 안 돼요.\n\n**③ 페이지 넘어갈 때 드래그 취소** — 페이지네이션된 리스트에서 항목을 잡고 다음 페이지로 넘어가는 순간, 페이지 전환으로 source DOM 이 unmount 되어 사라집니다. 브라우저는 \"드래그 대상이 사라졌네\" 하고 드래그 자체를 취소해 버려요.",
    ...
    cause: {
      ko: "HTML5 D&D 는 DOM 의 `draggable` 속성을 **드래그 시작 시점에 딱 한 번만 읽고**, 이후 속성이 바뀌어도 반응하지 않아요. 그래서 ① 처럼 React state 로 켰다 껐다 하는 방식과는 궁합이 안 맞습니다.\n\n그리고 D&D 표준은 source 노드가 드래그 내내 \"마운트 상태로 그대로 있을 것\" 을 가정합니다. source 가 도중에 unmount 되면 그 순간 drag session 을 곧장 cancel — 이게 ③ 의 원인이에요.\n\n② 비대칭 동작도 사실 ③ 과 같은 메커니즘입니다. chip 순서가 \"앞 → 뒤\" 로 바뀌면 React 가 key 기반 reconcile 로 source DOM 을 다른 슬롯으로 \"이동\" 시키는데, 브라우저 입장에선 그게 \"unmount 되었다가 다시 mount 된 것\" 과 비슷하게 보여 드래그 추적이 끊기는 거죠.\n\n결국 이런 quirks 가 작은 정렬 UI 에선 너무 빠르게 누적됩니다.",
    ...
    solution: {
      ko: "두 컴포넌트 모두 **pointer event 기반 드래그** 로 갈아 끼웠어요. HTML5 D&D 와 달리 pointer event 는 우리가 직접 다루는 거라, 위의 quirks 가 다 무관해집니다.\n\n**기본 흐름**: grip handle 에서 `pointerdown` 이 발생하면 document 레벨에 `pointermove` / `pointerup` 리스너를 임시로 붙입니다. move 마다 `elementFromPoint(clientX, clientY)` 로 \"지금 커서가 어떤 chip 위에 있는지\" 를 좌표로 직접 검사하고 (`closest(\"[data-chip-id]\")`), `pointerup` 시점에 source 와 target id 를 비교해서 `selectedIds` 배열을 splice 하고 onChange 호출.\n\n**페이지네이션 리스트의 트릭**: 사용자가 list 의 위/아래 edge (60px 영역) 위에 잠깐 머물면, 단순히 `setPage` 만 호출하지 않고 `apply()` 를 같이 호출해 source 를 **\"인접 페이지의 첫 / 마지막 자리\" 로 진짜로 reorder** 시킵니다. 그러면 페이지가 넘어가도 source 가 unmount 되지 않고, 새 페이지의 첫 항목으로 그대로 살아남죠.\n\n**주의**: `setPointerCapture` 는 안 씁니다. 잡는 순간 부모가 자식의 click 까지 흡수해 버려서, chip 의 \"× 제거\" 버튼이 안 눌리게 돼요.",
    ...
    keyInsight: {
      ko: "HTML5 native D&D 는 원래 \"OS 수준에서 이미지나 파일을 다른 앱으로 끌어다 놓는\" 시나리오에 맞춰 만들어진 표준이에요.\n\n반면 **같은 페이지 안에서 작은 항목 (chip, list item) 의 순서를 바꾸는** 용도에 쓰면, 표준 자체의 quirks 가 너무 빨리 쌓여요. state 토글이 안 맞는 `draggable`, source unmount 시 cancel, 자식 click 차단, 비대칭 reorder, custom 드래그 이미지 만들기 어려움 같은 것들이요.\n\n작은 정렬 UI 는 처음부터 **pointer event 로 직접 짜는 게** 결과적으로 코드도 짧아지고 동작도 더 일관됩니다. \"표준 API 가 있다\" 는 게 항상 \"이 상황에선 그 표준 API 를 써야 한다\" 와 같은 말은 아니라는 좋은 사례였어요.",
  ...
  },
  {
    ...
    definition: {
      ko: "이 사이트의 모든 이미지 영역 — 에디터 커버, 포스트 본문, Works 갤러리, Plate 에디터 패널 같은 — 에서 \"이미지가 깨졌을 때 (404 / CORS 등) `/images/placeholder.svg` 로 자동 교체\" 되는 통일된 fallback 규칙을 만들고 싶었어요.\n\nReact 컴포넌트로 직접 렌더하는 `<img onError>` 는 잘 동작했는데, **마크다운으로 렌더된 본문** (MarkdownRenderer) 과 **richtext 영역** 에서는 같은 `onError` 가 아예 발화하지 않더라고요. 그래서 그 영역에선 깨진 이미지가 그대로 사용자에게 노출됐습니다.",
    ...
    cause: {
      ko: "원인은 세 가지가 겹쳐 있었어요.\n\n**① React 합성 이벤트의 사각지대** — `dangerouslySetInnerHTML` 로 삽입된 DOM 은 React 의 reconciler 가 관리하지 않습니다. `onError` 같은 합성 이벤트 prop 은 그 영역엔 부착되지 않아서, native `addEventListener(\"error\")` 를 직접 붙여야만 잡을 수 있어요.\n\n**② 이미 끝난 이미지는 error 가 다시 발화하지 않음** — 이미지 listener 를 \"늦게\" 부착하면, 이미 로드 시도가 끝난 (성공이든 실패든) 이미지의 `error` 이벤트는 소급해서 다시 발화하지 않습니다. 그래서 `addEventListener` 만 붙여 놓는다고 \"이미 깨져 있던 이미지\" 까지 잡히는 게 아니에요.\n\n**③ 동적으로 추가되는 img** — richtext 영역은 사용자가 에디터 모드를 토글하거나 lazy 로딩 등으로 `<img>` 가 뒤늦게 추가되는 일이 많습니다. 처음에 `querySelectorAll(\"img\")` 한 번만 돌고 끝내면, 그 이후에 추가된 img 는 listener 가 없으니 그대로 깨진 채 남죠.",
    ...
    solution: {
      ko: "위의 세 가지를 한 번에 해결하는 `attachImageFallback(root)` 유틸을 만들어 MarkdownRenderer 와 `useRichtextEnhance` 두 곳에서 같이 쓰게 했어요.\n\n**① 컨테이너 안의 모든 img 에 listener 부착** — `data-fallback-bound` 속성으로 중복 부착을 막으면서 native `error` listener 를 붙입니다.\n\n**② 즉시 동기 체크** — 부착하자마자 `img.complete && img.naturalWidth === 0` 을 검사해요. 둘 다 true 면 \"로드는 끝났는데 width 가 0\" → 이미 실패한 이미지란 뜻이거든요. 이 경우엔 retroactive 이벤트가 안 오니까 **그 자리에서 즉시 placeholder 로 swap**.\n\n**③ MutationObserver 로 추가되는 img 도 추적** — `MutationObserver(root, { childList: true, subtree: true })` 를 등록해 두면, 이후에 새로 추가되는 img 도 자동으로 같은 처리를 받게 됩니다. richtext 가 동적으로 바뀌어도 안전해요.\n\n**보너스 트릭** — placeholder 로 swap 할 때 `removeAttribute(\"srcset\")` 도 같이 호출. srcset 이 남아 있으면 브라우저가 swap 한 src 보다 srcset 의 후보를 우선 시도해서 다시 깨지는 경우가 있거든요.\n\n반대로 React 컴포넌트로 직접 렌더되는 영역 (PostEditor cover, WorkEditor main/gallery, RelationPicker chip, Plate ImagePanel 등) 은 그냥 평범하게 `onError` + state swap 만 써도 같은 효과가 납니다. DOM 을 직접 만질 필요가 없으니 이쪽이 가장 깔끔하죠.",
    ...
    keyInsight: {
      ko: "이 사례 하나에서 React 와 native DOM 의 경계에 대한 사실 4가지를 정리할 수 있었어요.\n\n**① `dangerouslySetInnerHTML` 영역은 React 합성 이벤트의 사각지대** — 이벤트 위임이 안 되니까, native `addEventListener` 가 유일한 길.\n\n**② 이미 끝난 이미지는 `error` 가 다시 발화하지 않음** — listener 를 붙인 직후 `complete && naturalWidth === 0` 같은 동기 체크가 필수.\n\n**③ src 만 바꿔도 srcset 이 옛 후보를 다시 시도함** — swap 할 때 `removeAttribute(\"srcset\")` 도 같이 호출.\n\n**④ 동적 콘텐츠는 `querySelectorAll` 한 번 만으로는 못 잡음** — MutationObserver 로 점진적으로 추적해야 함.\n\n넷 다 따로 보면 사소해 보이는데, 이 중 하나라도 빠지면 \"왜 유독 이 이미지만 fallback 이 안 되지?\" 같은 함정에 빠지더라고요.",
  ...
  },
  {
    ...
    definition: {
      ko: "커버 이미지 그라데이션 에디터에선 각 stop (색상 정지점) 마다 ColorPicker 를 띄울 수 있게 만들어 놨어요. stop 핸들의 swatch 를 클릭하면 그 자리에 popover 가 떠서 색을 바꿀 수 있는 식이죠.\n\n근데 어느 stop 을 눌러도 **popover 가 항상 같은 좌표 (화면 좌상단 근처) 에** 떴어요. trigger 자체 (handle) 는 stop bar 위에 정확히 자기 자리에 보이는데, popover 만 위치가 완전히 엉뚱한 곳에 박힌 상태였습니다.",
    ...
    cause: {
      ko: "ColorPicker 의 popover 위치는 trigger 노드의 `getBoundingClientRect()` 결과를 기준으로 계산해요.\n\n이 컴포넌트는 render-prop 패턴 (사용자가 trigger 를 임의의 ReactNode 로 직접 넘기는 형태) 이라, 들어온 trigger 를 일관되게 측정하기 위해 `<span ref={triggerRef}>` 라는 wrapper 로 한 번 감싸 두었습니다.\n\n문제는 stop handle 들이 `position: absolute` 로 stop bar 위 좌표에 절대 위치로 박혀 있었다는 점이에요. **CSS 에서 자식이 `position: absolute` 면 그 자식은 \"normal flow\" 에서 빠져 나가서, 부모의 박스 크기에 전혀 기여하지 않습니다.**\n\n그러니 wrapper `<span>` 이 자기 자식 (handle) 의 크기를 못 잡고 0×0 으로 쪼그라들어 버렸고, 모든 stop 의 wrapper rect 가 똑같이 `(stopBar.left, stopBar.top, 0, 0)` 으로 측정되었습니다. 그래서 popover 가 다 같은 자리에 뜬 거였어요.",
    ...
    solution: {
      ko: "popover 위치 계산을 **wrapper 의 rect 가 아니라 wrapper 의 첫 자식 (`firstElementChild`) rect 를 우선** 쓰도록 한 줄만 고쳤어요.\n\nwrapper 가 0×0 으로 쪼그라들어도, 실제 trigger 역할을 하는 자식 (stop handle 이든 평범한 swatch button 이든) 은 자기만의 viewport 좌표를 멀쩡하게 돌려주거든요. 그 좌표를 기준으로 popover 를 띄우면 정확한 위치에 anchor 됩니다.\n\n자식이 없거나 hidden 인 경우 (rect 도 0×0) 를 위해서, 자식 rect 의 width/height 가 0 이면 wrapper rect 로 fallback 하게 해 뒀어요. 이 한 줄 변경으로 stop handle (absolute) 도, 평범한 swatch (normal flow) 도 둘 다 정확하게 anchor 됩니다.",
    ...
    keyInsight: {
      ko: "이 사례에서 일반화할 수 있는 교훈 3가지를 얻었어요.\n\n**① `position: absolute` 인 자식은 부모의 box 에 기여하지 않습니다.** render-prop 으로 임의의 자식을 받는 컴포넌트가 wrapper 의 `getBoundingClientRect()` 를 그대로 믿어 버리면, 자식이 absolute 인 순간 조용히 깨져 버려요.\n\n**② 측정 대상은 wrapper 보다 자식 (혹은 가장 가까운 visible descendant) 이 더 안전합니다.** 사용자가 어떤 CSS 로 자식을 배치하든 자식 자체는 viewport 좌표를 가지고 있고, wrapper 의 크기는 자식의 layout 에 영향을 받으니까요.\n\n**③ 디버깅 팁** — popover 가 엉뚱한 데에 뜨면, DevTools console 에서 `el.getBoundingClientRect()` 를 wrapper 와 자식 둘 다에 찍어 보세요. wrapper 가 0×0 으로 나오면 거기서 답이 나옵니다.",
