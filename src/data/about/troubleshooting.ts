...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  ...
  },
  {
    ...
    cause: {
      ko: "이 사이트는 번역 API 를 여러 회사 (provider) 에서 골라 쓸 수 있게 코드를 짜 두었습니다 — DeepL, Google, Gemini, Claude 등.\n\n그런데 \"여러 회사를 지원\" 한다고 다 좋은 게 아니었습니다. **실제 동작 흐름은 \"이 기능엔 무조건 DeepL 한 곳만 쓴다\" 처럼 한 번에 한 회사만 호출** 하고, 그게 실패하면 그냥 거기서 끝이었거든요. 다른 회사로 자동으로 넘어가는 \"대체 경로 (fallback)\" 가 없었습니다.\n\n게다가 API 키가 등록되지 않은 회사를 골라 둔 상태에서도, UI 에서 \"이 기능은 지금 못 씁니다\" 라고 숨기거나 비활성화하지 않았습니다. 그래서 **사용자가 절대 성공할 수 없는 버튼을 계속 누를 수 있었던** 상황이 됐습니다.",
      en: "The codebase supported multiple translation providers (DeepL, Google, Gemini, Claude, etc.).\n\nBut \"supporting multiple providers\" wasn't the same as \"using them\". **The actual flow was \"this feature always calls DeepL\" — one provider per feature, and if it failed, that was the end.** No automatic switch to a different provider, no \"fallback\".\n\nWorse, even when the configured provider had no API key, the UI didn't hide or disable the feature. So **users could keep clicking a button that was guaranteed to fail**.",
    },
    solution: {
      ko: "사이트 설정에서 **\"기본 회사 + 백업 회사 순서\"** 를 직접 구성할 수 있게 바꿨습니다.\n\n예: 기본 DeepL, 실패 시 백업 순서 [Gemini → Google → Claude]. 기본이 실패하면 백업 회사들을 순서대로 시도하고, 하나라도 성공하면 그 결과를 사용자에게 반환합니다. 모두 실패하면 그때야 사용자에게 에러 메시지를 보여줍니다.\n\nAPI 키가 등록되지 않은 회사는 **호출 자체를 시도하지 않고 건너뜁니다** — 의미 없는 네트워크 요청을 만들지 않으려고요.\n\n그리고 \"배치 번역\" 이라는 케이스도 따로 처리했습니다. \"배치 번역\" 은 한 번의 API 호출로 여러 텍스트를 동시에 번역하는 기능 (예: 글 제목 + 본문 + 요약 = 3개를 한 번에 보냄) 입니다. 이때 5개를 보냈는데 4개만 결과가 돌아오는 \"부분 실패\" 가 발생할 수 있는데, 이걸 그냥 4개로 받아 쓰면 한 줄이 빠진 채로 저장됩니다. 그래서 **입력 개수와 결과 개수를 비교해서 다르면 그 회사를 실패로 판정** 하고 다음 백업 회사로 넘어가게 했습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "이 사이트는 일반 OS 커서 대신 \"마우스를 따라다니는 작은 원\" 같은 커스텀 커서를 그립니다. 이 커서가 한 화면에 60번 다시 그려지는 동안 (60fps), 아래 두 가지 일을 동시에 처리합니다.\n\n**① 위치 보간** — 실제 마우스 위치까지 한 프레임마다 조금씩 이동시키는 부드러운 트래킹. 마우스가 갑자기 멀리 움직여도 커서가 \"천천히 따라가는\" 듯한 효과를 주는 부분.\n\n**② 마우스 아래 요소 판별** — 지금 커서가 \"클릭 가능한 버튼 위인지 / 텍스트 입력 영역인지 / 비활성 영역인지 / 잡아서 끌 수 있는 영역인지\" 를 확인해, 커서 모양을 그에 맞게 바꾸는 부분.\n\n둘 다 매 프레임 (약 16ms 마다) 같은 함수 루프 안에서 처리하다 보니, **상대적으로 무거운 ② 가 가벼운 ① 까지 함께 늦춰서 커서 움직임이 끊기듯 보이는 (stutter)** 현상이 생겼습니다.",
    ...
    cause: {
      ko: "②의 \"마우스 아래 요소 판별\" 을 위해 매 프레임 **`elementsFromPoint(x, y)`** 라는 브라우저 API 를 호출했습니다. 이 API 는 \"이 좌표 위에 겹쳐 있는 모든 HTML 요소들의 목록\" 을 z-index 역순으로 반환합니다.\n\n복잡한 레이아웃에서는 한 좌표 위에 수십~수백 개의 요소가 겹쳐 있을 수 있습니다 (배경 / wrapper / 카드 / 텍스트 / 인터랙션 영역 등이 다 쌓여 있으니까요). 그걸 매 프레임 다 훑다 보니, **한 프레임 16ms 중 몇 ms 가 ② 에만 소비** 됐습니다.\n\n①의 위치 보간은 코드 자체는 가볍지만, 같은 루프에 묶여 있어 ② 가 끝나기를 기다리느라 함께 늦어졌습니다. 결과적으로 둘 다 동시에 느려진 셈입니다.",
    ...
    solution: {
      ko: "두 작업을 **서로 다른 주기로 분리** 했습니다.\n\n**① 위치 보간** 은 그대로 매 프레임 (60fps) 실행 — 부드러움을 그대로 유지.\n\n**② 요소 판별** 은 60ms 간격으로 한 번만 실행 (이 패턴을 \"throttle\" 이라고 합니다 — 자주 호출되는 함수를 일정 시간에 한 번씩만 실행하도록 제한). 1초에 60번이 아니라 약 16번만 실행되니 한 프레임당 비용이 거의 0 에 가까워집니다.\n\n②를 늦추면 \"커서 모양 변경\" 이 60ms (4 프레임) 정도 늦어지지만, 사람 눈에는 4 프레임 정도의 지연은 거의 \"즉시\" 처럼 느껴집니다 — 클릭 가능한 버튼에 마우스를 올려도 모양이 즉시 바뀌는 것처럼 보입니다. 반면 ① (위치) 은 한 프레임만 늦어도 stutter 가 바로 눈에 띄니, **이쪽만 매 프레임을 사수** 한 게 핵심입니다.\n\n참고: 터치 기기는 애초에 마우스가 없으니 커스텀 커서 자체를 비활성화합니다.",
    ...
    keyInsight: {
      ko: "성능이 빡빡한 루프에 비용이 다른 두 작업을 같이 묶으면, **무거운 쪽이 가벼운 쪽까지 끌어내립니다**.\n\n해결의 출발점은 \"이 작업이 정말 매 프레임마다 필요할까?\" 라고 한 번 의심해 보는 것입니다.\n\n위치처럼 한 프레임만 늦어도 사람 눈에 바로 보이는 것은 **매 프레임 실행 (high-frequency)**, 요소 판별처럼 약간의 지연이 사람 눈에 안 보이는 것은 **낮은 빈도로 분리 (throttle / debounce)** — 같은 함수 안에 있는 작업도 비용과 \"인지 속도\" 에 맞게 주기를 다르게 가져가야 안 끊깁니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "에디터에서 글을 쓰는 동안 일정 주기로 임시 저장 (자동저장) 을 해 둬야, 갑자기 브라우저가 꺼지거나 실수로 페이지를 떠나도 작업물이 날아가지 않습니다.\n\n초기에는 이 임시 저장을 `localStorage` (브라우저가 자체적으로 가진 로컬 저장 공간) 에 직접 했는데, 시간이 지나면서 여러 문제가 한꺼번에 드러났습니다.\n\n**① 다른 기기 / 브라우저에서 안 보임** — 집 PC 에서 작성하던 글을 노트북에서 이어 쓰려 해도 임시 저장이 따라오지 않음.\n\n**② 새로고침만 해도 \"저장됨\" 알림이 뜸** — 실제로 글 내용은 하나도 안 바뀌었는데도 시스템이 \"변경됨\" 으로 인식해 매번 새 임시 저장을 만듦.\n\n**③ 무한 \"불러올까요?\" 알림** — 에디터를 다시 열면 \"이전에 저장된 임시본을 불러올까요?\" 가 뜨는데, \"무시\" 를 눌러도 새로고침하면 똑같은 내용으로 다시 물어봄.",
    ...
    cause: {
      ko: "원인이 세 가지가 겹쳐 있었습니다.\n\n**① localStorage 자체의 한계** — \"같은 브라우저, 같은 기기\" 안에서만 접근됩니다. 다른 PC 의 같은 사용자, 모바일 기기 등에서는 보이지 않습니다. 임시 저장을 다른 환경으로도 옮기려면 결국 서버에 저장해야 합니다.\n\n**② \"변경 없음\" 비교가 잘못됨** — 코드 안에서 \"마지막으로 저장된 내용\" 을 기억하는 변수의 초기값이 빈 문자열 `\"\"` 였는데, 실제 폼 상태를 JSON 으로 변환한 값은 빈 객체라도 `\"{}\"` 입니다. 두 값이 다르므로 mount 직후부터 시스템은 \"변경됨\" 으로 판단하고, 새로고침 = 처음 폼 mount 마다 매번 새 임시 저장을 만들었습니다.\n\n**③ \"무시한 임시본\" 을 기억하지 못함** — 사용자가 \"불러오기\" 알림을 무시해도, 그 임시본 자체는 DB 에 그대로 남습니다. 다음 진입 때 시스템은 \"같은 임시본\" 을 다시 발견하고 또 알림을 띄웠습니다. \"이미 무시한 거\" 라는 정보를 어디에도 안 적어 놨던 거죠.",
    ...
    solution: {
      ko: "원인 ①②③ 을 각각 해결했습니다.\n\n**① 저장소를 서버 DB 로 이전** — `localStorage` 사용을 완전히 제거하고, Supabase 의 `revisions` 테이블 (글의 \"버전 기록\" 테이블 — 이전에 저장된 모든 임시본을 row 단위로 보관) 을 유일한 저장소로 변경. 이제 다른 기기에서 로그인해도 진행 중이던 글이 그대로 따라옵니다.\n\n**② \"변경 없음\" 비교 기준값 수정** — \"마지막 저장된 내용\" 변수를 빈 문자열이 아니라 **폼 mount 직후의 실제 직렬화 값** 으로 초기화. 이제 \"진짜 변화\" 만 자동저장을 트리거합니다.\n\n**③ 무시한 임시본 추적** — 사용자가 \"무시\" 한 임시본의 식별값을 메모리상 `Set` (중복 없는 컬렉션) 에 저장. 다음에 시스템이 같은 임시본을 발견해도 Set 에 이미 들어 있으면 알림을 안 띄웁니다.\n\n**페이지 이탈 시 마지막 저장 보장** — 사용자가 브라우저를 닫거나 다른 페이지로 이동하는 순간에도 마지막 변경분이 안 날아가도록, **`navigator.sendBeacon`** (브라우저 종료 중에도 안전하게 서버로 데이터 전송하는 표준 API) 과 **`fetch({ keepalive: true })`** (페이지가 사라져도 요청을 끝까지 유지) 를 함께 사용.",
