...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  /* ── Backend / Admin ── */
  {
    ...
    cause: {
      ko: "처음 구현은 일반적인 CRUD 패턴 그대로, \"삭제 버튼 = DB 에서 row 즉시 제거\" 였습니다.\n\n그러다 admin 화면에 **체크박스로 여러 글을 한 번에 지우는 일괄 삭제 UI** 를 추가하게 됐는데, 만들어 놓고 보니 \"어, 이거 한 번 잘못 누르면 작성 중이던 글들이 한꺼번에 통째로 날아가겠는데?\" 라는 생각이 들었습니다. 단일 글 삭제만 있을 땐 잘 안 보이던 위험이, **여러 row 가 한 번의 클릭으로 함께 사라질 수 있는 구조** 가 되니 명확하게 드러난 것입니다.\n\n관리자가 한 명뿐이라 \"실수할 일이 있겠나\" 라고 넘기기 쉬운 부분이지만, 단일 관리자 환경이야말로 **한 번의 실수를 받아 줄 다른 사람도, 검토 단계도 없는 환경** 입니다. 발생 빈도가 낮더라도 한 번 발생했을 때의 비용이 비대칭적으로 크기 때문에, 일괄 삭제와 함께 \"되돌릴 수 있는 삭제\" 구조까지 같이 만들어 두기로 했습니다.",
      en: "The initial implementation followed the textbook CRUD pattern: \"delete button = remove the row from the DB immediately\".\n\nThen I added a **bulk-delete UI** to the admin page — checkboxes to remove multiple posts in one click. Once that was in place, the implication became obvious: \"one wrong click here could wipe several drafts at once\". A risk that's easy to miss with single-row deletes becomes glaring the moment **a single click can disappear multiple rows**.\n\nIt's tempting to dismiss this as \"a single admin won't make mistakes\", but a single-admin environment is precisely **the one with no second person and no review step to catch mistakes**. The frequency may be low, but the cost of a single occurrence is disproportionately high — so I decided to ship the bulk-delete UI alongside a \"reversible delete\" structure from day one.",
    },
    solution: {
