...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  /* ── Backend / Admin ── */
  {
    ...
    keyInsight: {
      ko: "삭제는 **'없앤다'가 아니라 '숨긴다'** 로 시작해야 합니다.\n\n돌이킬 수 없는 작업은 별도 단계로 한 번 더 분리하고, **기본 삭제는 언제든 되돌릴 수 있는 안전한 기본값** 이어야 합니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "AI 번역과 요약 기능은 외부 회사(DeepL, Gemini 등)의 API 를 호출해 처리합니다.\n\n테스트 중 `.env` 파일에서 API 키를 잠시 주석 처리했더니, 번역·요약을 시도할 때마다 **\"인증 실패\" 같은 에러 메시지가 사용자에게 그대로 노출**됐습니다.\n\n대부분 API 를 무료 plan 으로 쓰는 환경이라 **rate limit (시간당 호출 제한) 도달이나 일시적 장애는 실제 운영에서도 충분히 발생할 수 있는 시나리오** 였습니다.",
    ...
    cause: {
      ko: "여러 provider 를 지원하긴 했지만 **각 기능이 한 provider 에만 묶여 있고, 그게 실패해도 대체 경로가 없는 구조** 였습니다.\n\nAPI 키가 등록되지 않은 기능도 UI 에서 숨기거나 비활성화하지 않아서, **사용자가 절대 성공할 수 없는 기능을 계속 시도** 할 수 있었습니다.",
      en: "Multiple providers were supported, but **each feature was tied to a single provider with no fallback** — if it failed, there was no alternative path.\n\nFeatures missing an API key weren't hidden or disabled in the UI either, so **users could keep triggering operations that were guaranteed to fail**.",
    },
    solution: {
      ko: "사이트 설정에서 **\"기본 provider + 백업 우선순위 리스트\"** 를 구성할 수 있게 바꿨습니다. 기본이 실패하면 백업 리스트 (예: DeepL → Gemini → Google → Claude) 를 순서대로 시도해, 하나라도 성공하면 그 결과를 즉시 반환합니다.\n\nAPI 키가 없는 provider 는 **호출 자체를 시도하지 않고 자동으로 건너뛰어**, 의미 없는 네트워크 요청을 줄였습니다.\n\n또한 배치 번역에서 텍스트 5개를 보냈는데 결과가 4개만 돌아오는 \"불완전 응답\" 도 생깁니다. 이런 경우 **입력 개수와 결과 개수를 비교해 일치하지 않으면 그 provider 를 실패로 판단** 하고 다음 provider 로 fallback 합니다.",
      en: "Site settings now expose a **\"primary provider + backup priority list\"** configuration. If the primary fails, the backup list (e.g., DeepL → Gemini → Google → Claude) is tried in order, returning on the first success.\n\nProviders without an API key are **skipped before any network call** — no wasted requests on guaranteed-to-fail providers.\n\nBatch translation also has a subtle failure: send 5 texts, get 4 results back. The pipeline now **compares input vs. output count and treats any mismatch as a provider failure**, falling through to the next one.",
    },
    keyInsight: {
      ko: "외부 API 에 의존하는 기능은 **\"이 API 가 응답하지 않을 때 사용자에게 무엇이 보일까?\"** 를 설계 단계에서 미리 답해 둬야 합니다.\n\nprovider 가 완전히 죽지 않더라도 rate limit 만으로 호출이 실패할 수 있어, **에러를 그대로 노출할지 / 기능을 숨길지 / 대체 경로로 우회할지** 셋 중 하나를 사전에 정해 두는 게 안전합니다.",
  ...
  /* ── Frontend / Performance ── */
  {
    ...
    definition: {
      ko: "스팸 방지에 쓰는 reCAPTCHA 스크립트(784KB) 가 페이지 로드 직후 바로 다운로드되어, **LCP (페이지의 가장 큰 콘텐츠가 화면에 그려지는 시점) 17.1초, TTI (사용자가 클릭 등 인터랙션을 할 수 있게 되는 시점) 18.2초** 로 초기 렌더링이 매우 느려졌습니다.\n\n참고로 \"빠르다\" 의 기준은 LCP 2.5초 이하입니다. 17초는 사용자가 떠나기 충분한 시간이죠.",
    ...
    cause: {
      ko: "Google 공식 문서대로 reCAPTCHA 스크립트를 앱 시작 시점에 바로 불러왔습니다.\n\n그런데 reCAPTCHA 는 사실 **컨택트 폼에서 메시지를 보낼 때만 필요한 기능** 입니다. 즉, 페이지를 둘러보기만 하는 대부분의 사용자는 한 번도 안 쓰는 스크립트를 매번 다운받고 있었던 셈이죠.\n\n784KB 파일 다운로드 + 파싱 + 실행이 **메인 스레드를 점유**하면서, 정작 사용자가 봐야 할 콘텐츠 그리는 작업이 뒤로 밀렸습니다.",
    ...
    solution: {
      ko: "스크립트 로드 시점을 **\"앱 시작\" 에서 \"사용자가 처음 페이지에 인터랙션할 때 (클릭/스크롤/터치)\"** 로 늦췄습니다.\n\n실제 reCAPTCHA 가 필요한 컨택트 폼까지 도달하는 사용자는 거의 항상 그 전에 한 번 이상 인터랙션을 하므로, 폼을 누를 때쯤이면 이미 스크립트가 백그라운드에서 로드된 상태입니다.\n\n추가로 Google 서버와의 TCP/TLS 연결만 미리 열어 두는 **`<link rel=\"preconnect\">`** 를 head 에 넣어, 실제 다운로드가 시작될 때 connection setup 시간만큼 더 빨라지게 했습니다.",
    ...
    keyInsight: {
      ko: "외부 스크립트를 추가하기 전에 **\"이 스크립트가 페이지가 그려지기 전부터 필요한가?\"** 를 먼저 따져야 합니다.\n\n당장 안 쓰는 무거운 파일을 즉시 로드하면 사용자가 실제로 보는 콘텐츠가 수 초씩 늦어집니다. 사용자 인터랙션 직전까지는 일단 미루는 \"on-demand\" 가 거의 항상 더 좋은 기본값입니다.\n\n참고로 reCAPTCHA 는 **컨택트 폼에만** 적용했습니다. 댓글은 이미 비밀번호 + IP 해시 이중 인증으로 보호되니 captcha 까지 넣으면 **참여 허들만 높아지고 효과는 한정적** 이라 판단했습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "이 사이트는 마우스를 따라다니는 커스텀 커서 효과를 사용합니다. 이 커서는 두 가지 일을 동시에 합니다.\n\n**① 위치 보간** — 실제 마우스 위치까지 천천히 따라가는 부드러운 움직임 (LERP). ② **타입 판별** — 마우스 아래에 있는 요소를 보고 \"클릭 가능 / 텍스트 / 비활성 / 잡기 가능\" 중 어떤 모양으로 바꿀지 결정.\n\n둘 다 매 프레임(60fps 기준 약 16ms 마다) 같은 루프에서 처리하다 보니, 무거운 ② 가 가벼운 ① 까지 같이 느리게 만들어 **커서가 끊어지듯 움직이는 stutter** 가 발생했습니다.",
      en: "The site uses a custom cursor effect that follows the mouse. It does two things in tandem:\n\n**① Position interpolation** — smoothly easing toward the real mouse position (LERP). ② **Type detection** — looking at what's beneath the cursor and choosing a shape: clickable / text / disabled / grabbable.\n\nBoth ran together every frame (~16ms at 60fps) in the same loop, so the heavy ② dragged down the lightweight ① — producing a **stuttering, choppy cursor**.",
    },
    cause: {
      ko: "타입 판별을 위해 매 프레임 **`elementsFromPoint()`** 라는 브라우저 API 를 호출했습니다. 이 API 는 주어진 좌표 위에 쌓인 **모든 DOM 요소를 z-index 역순으로 모두 반환** 합니다.\n\n복잡한 레이아웃에서는 한 좌표 위에 수십~수백 개 요소가 겹쳐 있을 수 있고, 매 프레임 그걸 다 훑다 보니 **프레임당 수 ms 가 hit-test 에만 소비** 됐습니다. 같은 루프에 묶인 위치 보간 코드도 그만큼 늦게 실행돼, 결과적으로 둘 다 느려진 셈입니다.",
      en: "Type detection called the browser API **`elementsFromPoint()`** every frame. This API returns **every DOM element stacked at the given coordinate, ordered by z-index**.\n\nIn a complex layout, dozens or even hundreds of elements can overlap at a single point. Walking that list every frame burned **several ms per frame on hit-testing alone**, and since position interpolation lived in the same loop, it was delayed by the same amount — both ended up slow.",
    },
    solution: {
      ko: "두 작업을 **다른 주기로 분리** 했습니다.\n\n**위치 보간** 은 그대로 매 프레임 RAF 에서 실행 — 60fps 부드러움 유지.\n**hit-test** 는 60ms 간격으로 throttle — 1초에 60번이 아닌 약 16번만 실행.\n\n사람의 시각으로는 60ms 의 지연(약 4 프레임) 동안 커서 모양이 같다고 해서 **\"느리다\"고 느끼지 않습니다**. 클릭 가능 영역 위로 들어갔을 때 모양이 \"즉시\" 바뀌는 것처럼 보이죠. 반면 위치는 한 프레임만 끊겨도 바로 stutter 가 보이니, **이쪽만 60fps 를 사수** 한 게 핵심입니다.\n\n터치 기기는 마우스 자체가 없으니 커스텀 커서 전체를 비활성화합니다.",
      en: "**Split the two tasks across different cadences**:\n\n**Position interpolation** still runs in every RAF frame — keeping 60fps smoothness.\n**Hit-testing** is throttled to a 60ms interval — about 16 runs/sec instead of 60.\n\nA 60ms (≈ 4 frame) delay on cursor-shape changes is **imperceptible** to humans — the shape still appears to update \"instantly\" when you hover a clickable area. Position, on the other hand, breaks immediately on a single dropped frame — so **only that one had to stay at 60fps**.\n\nTouch devices don't have mice, so the custom cursor is disabled entirely there.",
    },
    keyInsight: {
      ko: "성능이 빡빡한 루프에 두 가지 작업을 같이 묶으면, **무거운 쪽이 가벼운 쪽까지 끌어내립니다**.\n\n해결의 출발점은 \"이 작업이 정말 매 프레임 필요할까?\" 라고 한 번 의심해 보는 것입니다.\n\n위치처럼 한 프레임만 늦어도 즉시 보이는 것은 **매 프레임 (high-frequency)**, DOM 탐색처럼 무겁지만 약간의 지연이 보이지 않는 것은 **낮은 빈도 (throttled / debounced)** — 같은 루프 안의 작업도 비용에 맞게 주기를 다르게 가져가야 합니다.",
      en: "When you couple two tasks in a tight loop, **the heavy one drags the light one down with it**.\n\nThe starting point is asking, \"does this really need to run every frame?\".\n\nWork like position updates — where a single late frame is visible as stutter — needs **per-frame (high-frequency)** updates. Heavy work like DOM traversal, where a small delay is invisible, can run **less often (throttled or debounced)**. Tasks in the same loop should be cadenced by their cost, not by convenience.",
    },
    comparisons: [
      {
  ...
  /* ── CSS / Styling ── */
  {
    ...
    definition: {
      ko: "다크 ↔ 라이트 테마를 부드럽게 전환하려고 모든 요소에 글로벌 transition (배경색·글자색 변화 애니메이션) 을 걸었더니, 정작 컴포넌트마다 따로 만든 **\"토글 펼침 / 메뉴 슬라이드 / 페이드인\" 애니메이션이 전부 무시** 되는 부작용이 생겼습니다.\n\n토글을 눌러도 부드럽게 펼쳐지지 않고 한 번에 \"툭\" 나타나고, 모달이 페이드인 없이 그냥 켜지는 상태였습니다.",
    ...
    cause: {
      ko: "글로벌 규칙은 `html[data-theme-ready] *` 에 걸려 있고, 이 선택자의 **specificity (CSS 우선순위)** 는 `(0,1,1)` 입니다. 컴포넌트의 단일 클래스 (예: `.modal`) 는 `(0,1,0)` 이라 항상 글로벌 규칙에 집니다.\n\n게다가 `transition` 은 shorthand 속성이라 \"나열한 속성만 덮어쓰는\" 게 아니라 **나열하지 않은 모든 속성의 transition 까지 통째로 초기화** 합니다. 즉, 글로벌에서 `transition: background-color 0.3s` 한 줄을 쓴 순간, 그 요소의 `max-height transition`, `opacity transition`, `transform transition` 이 전부 사라집니다.",
    ...
    solution: {
      ko: "컴포넌트의 transition 이 글로벌 규칙을 이기도록, **두 클래스를 결합한 복합 선택자 (specificity `(0,2,0)`)** 를 사용했습니다.\n\n예: `.modal { transition: opacity 0.3s }` 대신 `.modalWrap .modal { transition: opacity 0.3s }`. specificity 가 `(0,2,0)` > `(0,1,1)` 이므로 글로벌 shorthand 를 안전하게 덮어씁니다.\n\n프로젝트 CLAUDE.md 에도 이 패턴을 \"transition 이 동작하지 않으면 가장 먼저 의심할 것\" 으로 명시해 같은 함정을 반복하지 않도록 했습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "Plate 에디터에서 토글, 콜아웃, 열블록을 작성하고 저장한 뒤 페이지를 다시 열면, **블록 자체는 남아있는데 내부 콘텐츠만 전부 비어 있는** 이상한 현상이 생겼습니다.\n\n예를 들어 \"📝 메모\" 라는 콜아웃 안에 본문 두 줄을 적어 저장했는데, 다시 열면 \"📝 메모\" 박스만 남고 본문은 사라진 상태였죠. 토글·콜아웃·열블록 4종 모두에서 동일하게 발생.",
    ...
    cause: {
      ko: "Plate 는 저장된 HTML 을 다시 에디터 노드 (Slate 트리) 로 복원할 때 \"deserializer\" 라는 함수를 호출합니다. 각 블록 플러그인은 자기 deserializer 의 `parse` 함수를 따로 구현할 수 있는데, 거기서 **`children: []` (빈 자식 배열) 를 명시적으로 반환** 하고 있던 게 원인이었습니다.\n\nPlate 의 deserializer 는 두 가지 모드로 동작합니다:\n\n- `parse` 가 `children` 을 **반환하지 않으면** → HTML 안쪽 자식 노드를 **자동으로 재귀 파싱** (default)\n- `parse` 가 `children` 을 **명시적으로 반환하면** → 그 값을 그대로 사용, 자동 파싱 안 함\n\n즉, `children: []` 라고 쓰는 순간 \"내가 직접 빈 배열을 설정했으니 자동 파싱 하지 마\" 라는 의미가 되어, **HTML 안에 본문이 멀쩡히 있어도 무시** 하고 빈 채로 복원했던 것입니다.",
    ...
    solution: {
      ko: "각 deserializer 의 `parse` 반환 객체에서 **`children: []` 한 줄만 삭제** 했습니다.\n\n`children` 필드가 없으면 Plate 의 기본 동작이 발동해, `<div>` 안의 HTML 을 자동으로 재귀 파싱해서 Slate 노드 트리로 변환합니다. 4개 플러그인 (토글, 콜아웃, 열 그룹, 열 아이템) 에 동일한 패치를 적용해 모두 해결.",
    ...
    keyInsight: {
      ko: "프레임워크의 **\"기본 동작 vs 명시적 동작\"** 의도를 이해하지 못하면, 안전해 보이는 코드 한 줄이 프레임워크의 자동화를 통째로 끄는 일이 생깁니다.\n\nPlate 의 `parse` 에서 `children` **생략** 은 \"자동 파싱 켜짐\", **명시** 는 \"수동 제어 — 자동 파싱 꺼짐\" 입니다. 빈 배열 `[]` 도 \"자식 없음\" 이라는 **의도적 선언** 으로 해석됩니다.\n\n\"명시적이 항상 더 안전하다\" 는 직관이 항상 옳지는 않다는 점, 그리고 라이브러리의 default 가 어떤 가정을 깔고 있는지 한 번은 들여다 보는 게 좋다는 교훈이 됐습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "초기 자동저장은 `localStorage`(브라우저 로컬 저장소) 에 직접 저장했지만, 시간이 지나면서 여러 문제가 한꺼번에 드러났습니다.\n\n① **탭이나 기기를 옮기면 임시 저장이 따라오지 않음** (집 PC 에서 쓰던 글이 노트북에서 안 보임), ② **새로고침만 해도 \"실제 변경이 없는데\" 저장이 트리거됨**, ③ **\"불러올까요?\" 안내를 무시해도 새로고침하면 같은 내용으로 또 묻는 무한 루프** 등.",
      en: "The initial auto-save wrote directly to `localStorage` (browser-local storage), but several issues compounded over time:\n\n① **Drafts didn't follow you across tabs or devices** (a draft from your home PC was invisible on your laptop), ② **a plain refresh triggered a \"save\" even when nothing changed**, and ③ **dismissing a \"Restore draft?\" prompt only to be re-prompted on refresh with the same content** — an infinite loop.",
    },
    cause: {
      ko: "여러 원인이 겹쳐 있었습니다.\n\n첫째, **localStorage 자체의 한계** — 같은 브라우저, 같은 기기에서만 접근됩니다. 둘째, \"마지막으로 저장된 내용\"을 기억하는 ref 의 초기값이 빈 문자열 `\"\"` 인데, 폼 상태를 JSON 으로 변환한 값(`JSON.stringify(form)`) 은 빈 객체라도 `\"{}\"` 라서 **mount 직후부터 항상 \"변경됨\" 으로 잡혔습니다**. 셋째, 사용자가 \"무시\" 한 리비전의 snapshot 을 추적하지 않아 **동일 내용을 매번 새 리비전으로 다시 묻는** 상황이 발생했습니다.",
      en: "Several causes compounded:\n\nFirst, **a fundamental limitation of localStorage** — same browser, same device only. Second, the ref tracking \"last saved content\" started as an empty string `\"\"`, but `JSON.stringify(form)` produces `\"{}\"` even for an empty form — so **the diff was never zero from mount onward, marking the form as \"changed\" forever**. Third, dismissed revision snapshots weren't tracked, so the **same content kept resurfacing as a fresh restore prompt** on every reload.",
    },
    solution: {
      ko: "3단계로 개선했습니다.\n\n**① localStorage 완전 제거** — DB 의 `revisions` 테이블을 유일한 저장소로. 이제 다른 기기에서 로그인해도 진행 중이던 글이 그대로 따라옵니다.\n\n**② `lastAutoSaveJson` 초기값 수정** — 폼 mount 직후의 실제 직렬화 값으로 초기화해, \"변경 없음\" 상태를 정확히 비교 가능하게.\n\n**③ dismissed snapshot 을 `Set` 으로 추적** — 한 번 무시한 내용은 동일 hash 가 들어와도 다시 안 묻습니다.\n\n페이지를 떠날 때(브라우저 닫기, 다른 페이지로 이동) 마지막 변경분이 날아가지 않도록 **`sendBeacon` + `fetch({ keepalive: true })`** 조합으로 마지막 저장을 보장했습니다.",
      en: "Three-stage cleanup:\n\n**① Removed localStorage entirely** — the DB `revisions` table is now the sole source of truth. Drafts follow you across devices the moment you log in.\n\n**② Fixed the `lastAutoSaveJson` initial value** — initialized from the actual serialized form right after mount, so \"unchanged\" can be compared accurately.\n\n**③ Track dismissed snapshots in a `Set`** — once you dismiss a draft, identical hashes are never re-prompted.\n\nFor the page-leave moment (close tab, navigate away), `sendBeacon` + `fetch({ keepalive: true })` guarantees the final save reaches the server even after the page is gone.",
    },
    keyInsight: {
      ko: "자동저장의 핵심은 \"언제 저장할까?\" 보다 **\"언제 저장하지 않을까?\"** 입니다.\n\n비교 기준값을 정확히 초기화해 ② **\"변경 없는데 저장\"** 을 막고, ③ **사용자가 무시한 내용은 다시 묻지 않도록** 추적해야 의미 있는 리비전만 쌓입니다. 그렇지 않으면 한 시간 작업에 \"단어 하나 추가\" 짜리 리비전이 수십 개 쌓여 정작 되돌아가고 싶은 시점을 못 찾습니다.",
      en: "Auto-save is less about **\"when to save\"** and more about **\"when NOT to save\"**.\n\nGet the baseline value right so ② **\"save with no real change\"** stops, and ③ **dismissed content stays dismissed** — that's how the history stays meaningful. Otherwise an hour of writing leaves dozens of \"added one word\" revisions and you can't find the checkpoint you actually wanted.",
    },
  },
  {
  ...
  },
  {
    ...
    definition: {
      ko: "홈 화면 CTA 섹션의 \"Contact / Resume\" 버튼은 호버 시 뒤에 있는 3D 커피잔이 흐릿하게 비치는 \"유리창 너머\" 효과 (`backdrop-filter: blur()`) 를 사용합니다.\n\n그런데 Chrome 에서만 **이 blur 효과가 전혀 안 보이고**, 단순한 반투명 배경 tint 만 약하게 표시됐습니다. DevTools \"Computed\" 탭에서 `backdrop-filter` 속성은 분명히 적용돼 있는데, 실제 샘플링이 일어나지 않은 셈입니다.",
    ...
    cause: {
      ko: "홈 진입 애니메이션이 `.home` 래퍼를 `y: '100vh' → 0` 으로 슬라이드 업 하는 **transform 기반** 이었습니다.\n\n애니메이션이 끝난 뒤에도 framer-motion 이 `transform: translate3d(0,0,0)` 와 `will-change` hint 를 유지했습니다. 이게 작은 문제 같지만, **`.home` 이 자체 compositing layer (브라우저 내부에서 별도 GPU 텍스처) 로 승격** 되는 트리거가 됩니다.\n\n그러면 하위 버튼의 `backdrop-filter` 는 그 layer **경계 안쪽** 만 샘플링할 수 있고, layer 바깥에 있는 3D 커피 canvas 의 픽셀은 \"뒤에 아무것도 없는 것\" 처럼 처리됩니다.\n\n추가로 `-webkit-backdrop-filter` 접두사가 Chrome 의 declaration 파싱을 꼬이게 만드는 케이스도 있어, 일부 환경에서 효과를 더 약화시켰습니다.",
    ...
    solution: {
      ko: "진입 애니메이션을 **`y` (transform 기반) → `marginTop` (layout 기반)** 으로 교체했습니다.\n\nmargin/padding/width 같은 layout 속성은 GPU 가 아닌 CPU 에서 처리되며 compositing layer 를 만들지 않습니다. 덕분에 `.home` 이 일반 layer 로 돌아오고, 하위 버튼의 `backdrop-filter` 가 layer 경계 너머의 커피 canvas 까지 정상 샘플링하게 됩니다.\n\n추가로 `.home` 의 `border-radius` + `overflow: clip` 조합 등 다른 layer 승격 트리거도 제거하고, `-webkit-backdrop-filter` 접두사는 삭제했습니다 (현대 Chrome 은 표준 `backdrop-filter` 만 사용).",
  ...
  },
  {
    ...
    definition: {
      ko: "PostCard 를 클릭해 포스트 상세 페이지로 이동할 때, 이미지가 카드 크기에서 상세 페이지의 hero 영역으로 부드럽게 \"모핑\" 됩니다. 그런데 다음 두 가지 버그가 한 번에 발생했습니다.\n\n**① hold 단계에서 무한 정지** — 이미지가 hero 크기로 축소된 뒤, 오버레이가 사라지지 않고 영원히 화면 위에 남았습니다.\n\n**② skeleton 노출** — 모핑이 끝난 직후 그 아래로 로딩 스켈레톤이 잠깐 그대로 보이는, 어색한 \"이미지 축소 → 스켈레톤 → 실제 페이지\" 시퀀스가 만들어졌습니다.",
    ...
    cause: {
      ko: "두 원인이 따로따로 있었습니다.\n\n**① 무한 hold** — 원래 설계는 `expand → morph(히어로 크기) → hold` 까지 자동 진행하고, 새 페이지의 hero `<motion.div>` 에 걸린 `onAnimationStart` 콜백에서 `endTransition()` 을 호출해 오버레이를 dismiss 하는 구조였습니다.\n\n그런데 hero 의 `initial` 과 `animate` 둘 다 `opacity: 1` 이라 (전환 중일 때만 즉시 보이도록), framer-motion 입장에선 \"애니메이션 시작 값 = 끝 값\" 즉 \"실제로 변하는 게 없는 상태\" 로 판정합니다. 그러면 **`onAnimationStart` 가 아예 발화하지 않아** 콜백 안의 `endTransition()` 이 호출되지 않고, phase 가 영원히 hold 에 갇혔습니다.\n\n**② skeleton 노출** — morph 가 클릭 후 고정 타이밍 (약 1초) 으로 진행되다 보니, **새 페이지가 준비되기 전에 오버레이가 먼저 작아졌습니다**. 그 사이 빈 자리는 Next.js 의 `loading.tsx` (Suspense fallback) 인 스켈레톤이 차지해, 사용자에게 깜빡 노출됐습니다.",
    ...
    solution: {
      ko: "전환 상태 머신을 재설계해 두 문제를 함께 해결했습니다.\n\n**① endTransition 트리거를 다른 메커니즘으로 변경** — `onAnimationStart` 의존을 제거하고, **새 페이지 컴포넌트의 `useEffect` 에서 mount 시점에 `endTransition()` 을 호출** 하도록 변경. 이건 \"애니메이션 동작 여부\" 와 무관하게 \"새 페이지가 마운트되었다\" 는 명확한 신호입니다.\n\n또한 어떤 이유로든 endTransition 이 호출되지 않을 경우를 대비해 **`SAFETY_MS=5000` 안전망 타이머** 를 PageTransitionProvider 에 추가 — 5초 안에 신호가 안 오면 강제 dismiss.\n\n**② backdrop 을 hold 동안 fullscreen 유지** — 기존엔 backdrop 도 hero 영역만큼만 덮었지만, 이제는 hold 단계 동안 backdrop 이 화면 전체를 덮고 있어, morph 후에도 그 아래의 스켈레톤이 보이지 않습니다.\n\n빠른 mount (데이터 캐시 hit) 케이스를 위해 `endRequestedRef` flag 도 추가했습니다. 새 페이지가 morph 보다 먼저 준비되면 hold 를 건너뛰고 곧바로 done 단계로 진입.",
    ...
    keyInsight: {
      ko: "두 가지 교훈을 얻었습니다.\n\n**① 애니메이션 라이프사이클 콜백을 critical state transition 의 단독 트리거로 쓰지 말 것.**\n\n`onAnimationStart` / `onAnimationComplete` 같은 콜백은 \"값이 실제로 변하지 않을 때\" 발화하지 않을 수 있고, 라이브러리 버전이나 렌더링 타이밍에 따라 silently fail 합니다. 항상 `useEffect` 기반 fallback 이나 setTimeout 안전망과 함께 설계해야 \"안 불러도 죽지는 않게\" 만들 수 있습니다.\n\n**② 모핑 전환에선 \"오버레이가 작아지면 그 아래가 노출된다\" 는 시각 계약을 잊지 말 것.**\n\nSuspense fallback (스켈레톤) 이 있는 환경에서 \"morph-into-hero\" 식 전환을 설계할 때, 새 페이지가 준비되기 전에 morph 가 끝나면 스켈레톤이 노출됩니다.\n\n해결은 두 가지뿐 — (a) backdrop 으로 morph 후에도 화면 전체를 덮어두거나, (b) 새 페이지 mount 시점까지 morph 자체를 지연시키거나. 둘 다 본질은 **\"오버레이의 시각 상태가 실제 페이지 상태와 동기화되도록 보장\"** 하는 것입니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "`/posts` 페이지의 bento 레이아웃은 5가지 비율의 카드를 한 grid 안에 섞어 씁니다 — 가로형 wide, 21:9 banner, 1:1 square, 3:4 portrait, 16:10 standard.\n\n그런데 일반 CSS Grid 만으로는 한 row 의 높이가 \"그 row 안에서 가장 큰 카드\" 기준으로 잡혀, **작은 square 카드 아래에 큰 portrait 카드와의 높이 차이만큼 빈 공간** 이 생겼습니다. 핀터레스트 같은 진짜 masonry 처럼 빈틈 없이 채워지지 않은 거죠.\n\n`grid-auto-flow: dense` (작은 셀이 빈자리 backfill) 만 추가해도 가로 빈자리는 어느 정도 채워지지만, **세로 잔여 공간** 은 여전히 처리되지 않았습니다.",
    ...
    cause: {
      ko: "CSS Grid 의 `grid-template-rows: auto` (또는 고정 비율) 는 한 row 의 높이를 \"그 row 의 자식 중 가장 큰 것\" 으로 묶어 정렬합니다.\n\n즉, 한 row 에 1:1 square 와 3:4 portrait 가 같이 들어가면 row 자체가 portrait 높이로 늘어나면서 square 아래에 dead space 가 생깁니다. row 가 \"한 단위 = 한 카드 높이\" 로 묶여 있어서 미세한 잔여 공간을 다른 카드가 침범할 수 없는 구조였던 거죠.",
    ...
    solution: {
      ko: "**진짜 masonry 를 CSS Grid + JS 하이브리드** 로 구현했습니다.\n\nCSS 쪽에선 `grid-auto-rows: 1px` 로 row 단위를 1px 까지 잘게 쪼개 두고, `grid-auto-flow: dense` 와 `gap` 만 지정합니다.\n\nJS (useEffect) 에선 모든 카드의 실제 높이 (`firstElementChild.scrollHeight`) 를 측정한 뒤, `span = ceil((높이 + gap) / (rowUnit + gap))` 공식으로 각 카드가 차지할 row 수를 계산해 `style.gridRow = span N` 으로 동적 부여합니다. row 단위가 1px 이라 카드 높이가 매우 정밀하게 packing 됩니다.\n\n폰트나 이미지가 늦게 로드되면 처음 측정한 높이가 어긋나기 때문에, **`ResizeObserver`(grid 크기 변화) 와 이미지 `onLoad` 두 시점 모두에서 재계산** 합니다.\n\n모바일 (≤ 640px) 에선 화면이 좁아 variant 조합 자체가 어색해지므로, 모든 카드를 단일 16:10 비율로 통일하고 JS 측정도 비활성화 — 비용을 줄였습니다.",
    ...
    keyInsight: {
      ko: "CSS-only masonry 는 아직 실험적 단계입니다. `grid-template-rows: masonry` 는 Firefox 외엔 미지원이라 Chrome 에서 못 씁니다.\n\n빈틈 없이 packing 하는 사실상의 표준은 **row track 을 1px 단위로 잘게 쪼갠 뒤 JS 측정 높이로 span 을 부여** 하는 하이브리드입니다.\n\n측정엔 `firstElementChild.scrollHeight` 가 가장 정확합니다 — 컨테이너 자체의 padding 이 합산되지 않아 픽셀이 \"진짜 콘텐츠 높이\" 와 맞아떨어지죠. 그리고 이미지 `onLoad` 와 `ResizeObserver` 두 시점에서 모두 재계산해야 폰트나 이미지가 늦게 로드돼도 잘못된 span 이 굳어지지 않습니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "관련글 chip 순서 변경 (RelationPicker) 과 작품 정렬 페이지네이션 (SortOrderDragList) 두 곳에서 \"드래그 앤 드롭\" 으로 항목을 정렬할 수 있게 만들었는데, HTML5 표준 D&D 를 쓰니 다음 세 가지 문제가 한꺼번에 발생했습니다.\n\n**① 드래그가 시작 안 됨** — \"드래그 중인 chip 만 draggable 속성 켜기\" 패턴 (`draggable={dragId === id}`) 이 React 의 batching 때문에 DOM 반영이 한 박자 늦어, 사용자가 처음 잡으려는 순간 `draggable=false` 인 상태로 `dragstart` 가 발사되어 무시됨.\n\n**② 비대칭 동작** — 같은 코드인데 \"뒤 chip 을 앞으로\" 는 잘 되고 \"앞 chip 을 뒤로\" 만 작동 안 함.\n\n**③ 페이지 전환 시 자동 cancel** — 페이지네이션된 리스트에서 source 항목을 잡고 다음 페이지로 넘어가는 순간, 페이지 전환으로 source DOM 이 unmount → 브라우저가 \"드래그 대상이 사라졌다\" 며 drag 자체를 취소.",
    ...
    cause: {
      ko: "HTML5 D&D 는 DOM `draggable` 속성을 **드래그 시작 시점에 단 한 번만 읽고**, 이후 속성이 바뀌어도 반응하지 않습니다. 그래서 ① 처럼 React state 로 토글하는 방식과 잘 안 맞습니다.\n\n또 D&D 표준은 source 노드가 mount 된 상태로 유지될 것을 가정합니다. source 가 unmount 되면 즉시 drag session 을 cancel — 이게 ③ 의 원인입니다.\n\n② 비대칭은 본질적으로 ③ 과 같은 메커니즘입니다. chip 순서가 \"앞→뒤\" 로 바뀌면 React 가 key 기반 reconcile 로 source DOM 을 다른 슬롯으로 \"옮기는\" 동작을 하는데, 이게 브라우저 입장에선 \"DOM 이 unmount 되었다 다시 mount 되었다\" 와 비슷해 drag 가 끊깁니다.\n\n작은 항목 정렬에 D&D 를 쓰면 이런 quirks 의 합이 너무 큽니다.",
    ...
    solution: {
      ko: "두 컴포넌트 모두 **pointer event 기반 드래그** 로 교체했습니다. HTML5 D&D 와 달리 pointer events 는 우리가 직접 제어하므로 위 quirks 가 모두 사라집니다.\n\n**기본 흐름**: grip handle 에서 `pointerdown` 이 발생하면 document 레벨에 `pointermove` / `pointerup` 리스너를 부착. move 마다 `elementFromPoint(clientX, clientY)` 로 \"커서가 지금 어떤 chip 위에 있는지\" 를 좌표로 직접 검출 (`closest(\"[data-chip-id]\")`). `pointerup` 에서 source 와 target id 를 비교해 `selectedIds` 배열을 splice → onChange.\n\n**페이지네이션 리스트의 트릭**: 사용자가 list 의 위/아래 edge (60px 영역) 위에 머물면, **단순히 setPage 만 하지 않고** `apply()` 를 호출해 source 를 \"인접 페이지의 첫/끝 슬롯\" 으로 실제로 reorder. 그러면 다음 페이지로 넘어가도 source 가 unmount 되지 않고 새 페이지의 첫 항목으로 살아남습니다.\n\n**주의**: `setPointerCapture` 는 **사용하지 않습니다**. 잡는 순간 부모가 자식의 click 까지 흡수해 chip 의 \"× 제거\" 버튼이 클릭되지 않습니다.",
    ...
    keyInsight: {
      ko: "HTML5 native D&D 는 \"이미지나 파일을 OS 수준에서 다른 앱으로 끌어다 놓는\" 시나리오에 최적화돼 있습니다.\n\n반면 **같은 페이지 안에서 작은 항목 (chip, list item) 순서를 바꾸는** 용도에는 표준 자체의 quirks 합이 너무 큽니다 — state 토글로 컨트롤되는 `draggable`, source unmount 시 cancel, 자식 click 차단, 비대칭 reorder, custom ghost 이미지 어려움 등.\n\n작은 정렬 UI 는 처음부터 **pointer events 로 직접 짜는 게** 결과적으로 코드도 짧고 동작도 일관됩니다. \"표준 API 가 있다\" 는 게 항상 \"표준 API 를 쓰는 게 맞다\" 와 동의어는 아니라는 좋은 사례입니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "이 사이트의 모든 이미지 영역 — 에디터 커버, 포스트 본문, Works 갤러리, Plate 에디터 패널 등 — 에서 \"이미지가 깨졌을 때 (404, CORS 등) `/images/placeholder.svg` 로 자동 교체\" 하는 통일된 fallback 을 만들고 싶었습니다.\n\nReact 컴포넌트로 직접 렌더하는 `<img onError>` 는 잘 작동했지만, **마크다운으로 렌더된 본문** (MarkdownRenderer) 과 **richtext 영역** 에서는 같은 `onError` 가 전혀 발화하지 않아 깨진 이미지가 그대로 사용자에게 노출됐습니다.",
    ...
    cause: {
      ko: "원인은 세 가지가 겹쳐 있었습니다.\n\n**① React 합성 이벤트의 사각지대** — `dangerouslySetInnerHTML` 로 삽입된 DOM 은 React 의 reconciler 가 관리하지 않습니다. `onError` 같은 합성 이벤트 prop 은 그 영역에 부착되지 않으니, 이론상 native `addEventListener(\"error\")` 를 직접 써야 합니다.\n\n**② 이미 끝난 이미지는 error 가 다시 발화하지 않음** — listener 를 \"늦게\" 부착하면, 로드 시도가 이미 끝난 (성공이든 실패든) 이미지의 `error` 이벤트는 retroactive 하게 발화하지 않습니다. 그래서 `addEventListener` 만으로는 \"이미 깨져 있는 이미지\" 는 영영 못 잡습니다.\n\n**③ 동적으로 추가되는 img** — richtext 영역은 사용자가 에디터를 토글하거나 lazy 로드 등으로 `<img>` 가 나중에 추가되는 케이스가 많습니다. 처음에 `querySelectorAll(\"img\")` 한 번 돌고 끝내면, 이후 추가된 img 는 listener 가 없어 그대로 깨진 채 남습니다.",
    ...
    solution: {
      ko: "위 세 가지를 한꺼번에 해결하는 `attachImageFallback(root)` 유틸을 만들고, MarkdownRenderer 와 `useRichtextEnhance` 두 곳에 도입했습니다.\n\n**① 컨테이너 내 모든 img 에 listener 부착** — `data-fallback-bound` 속성으로 중복 부착을 막으면서 native `error` listener 를 부착.\n\n**② 즉시 동기 체크** — 부착 직후 `img.complete && img.naturalWidth === 0` 을 검사. 둘 다 true 면 \"이미 로드 끝났는데 width 가 0\" 이라는 뜻 = 이미 실패한 이미지. 이 경우는 retroactive event 가 안 오므로 **즉시 placeholder 로 swap**.\n\n**③ MutationObserver 로 incremental 추적** — `MutationObserver(root, { childList: true, subtree: true })` 를 등록해 이후 추가되는 img 도 자동으로 동일 처리. richtext 가 동적으로 변해도 안전.\n\n**추가 트릭** — placeholder 로 swap 할 때 `removeAttribute(\"srcset\")` 도 함께 호출. srcset 이 남아 있으면 브라우저가 swap 한 src 보다 srcset 후보를 우선 시도해 다시 깨지는 케이스가 있어 차단.\n\nReact 컴포넌트로 렌더되는 영역 (PostEditor cover, WorkEditor main/gallery, RelationPicker chip, Plate ImagePanel 등) 은 평범하게 `onError` + state swap 으로 동일 효과 — DOM 직접 조작이 필요 없으니 가장 깔끔합니다.",
    ...
    keyInsight: {
      ko: "이 한 사례에서 React 와 native DOM 사이의 경계에 대한 4가지 사실을 모았습니다.\n\n**① `dangerouslySetInnerHTML` 영역은 React 합성 이벤트의 사각지대** — 이벤트 위임이 없으니 native `addEventListener` 가 유일한 선택지.\n\n**② 이미 완료된 이미지는 `error` 가 retroactive 하게 안 옴** — listener 부착 직후 `complete && naturalWidth === 0` 동기 체크가 필수.\n\n**③ src 만 바꾸면 srcset 이 \"옛 후보\" 를 다시 시도** — swap 할 때 `removeAttribute(\"srcset\")` 도 잊지 말 것.\n\n**④ 동적 콘텐츠는 querySelectorAll 단발로 못 잡음** — MutationObserver 로 incremental 추적이 필요.\n\n네 가지 모두 따로 보면 사소하지만, 셋만 충족해도 \"왜 이 이미지만 fallback 이 안 되지?\" 라는 함정이 생깁니다.",
  ...
  },
  {
    ...
    definition: {
      ko: "커버 이미지 그라데이션 에디터에서 각 stop (색상 정지점) 마다 ColorPicker 를 띄울 수 있게 만들었습니다. stop 핸들의 swatch 를 클릭하면 그 자리에 popover 가 떠서 색을 바꿀 수 있는 식이죠.\n\n그런데 어느 stop 을 클릭해도 **popover 가 항상 같은 좌표 (화면 좌상단 근처) 에** 떴습니다. trigger 자체 (handle) 는 stop bar 위 정확한 위치에 보이는데, popover 만 위치가 완전히 어긋난 상태였습니다.",
    ...
    cause: {
      ko: "ColorPicker 의 popover 위치는 trigger 노드의 `getBoundingClientRect()` 결과를 기준으로 계산합니다.\n\nrender-prop 패턴이라 사용자가 임의의 trigger ReactNode 를 넘길 수 있고, 컴포넌트는 그걸 일관되게 측정하려고 `<span ref={triggerRef}>` wrapper 한 번 감싸 두었습니다.\n\n문제는 stop handle 들이 `position: absolute` 로 stop bar 위에 절대 좌표로 배치되어 있다는 점이었습니다. **CSS 에서 자식이 `position: absolute` 면 그 자식은 \"normal flow\" 에서 빠져 부모의 박스 크기에 기여하지 않습니다.**\n\n그래서 wrapper `<span>` 자체가 0×0 으로 collapse 됐고, 모든 stop 의 wrapper rect 가 똑같이 `(stopBar.left, stopBar.top, 0, 0)` 으로 측정되어 popover 가 모두 같은 위치에 떴습니다.",
    ...
    solution: {
      ko: "popover 위치 계산을 **wrapper 의 rect 가 아니라 wrapper 의 첫 번째 자식 (`firstElementChild`) rect 를 우선** 사용하도록 수정했습니다.\n\nwrapper 가 0×0 으로 collapse 되어도, 실제 trigger 역할을 하는 자식 (stop handle 이든 일반 swatch button 이든) 은 자기만의 viewport 좌표를 정상적으로 반환합니다. 그걸 기준으로 popover 를 띄우면 정확한 위치에 anchor 됩니다.\n\n자식이 없거나 hidden 인 케이스를 위해 자식 rect 의 width/height 가 0 이면 wrapper rect 로 fallback. 이 한 줄 변경으로 stop handle (absolute) / 일반 swatch (normal flow) 두 케이스 모두 정확히 anchor 됩니다.",
    ...
    keyInsight: {
      ko: "이 사례에서 얻은 일반화 가능한 교훈 3가지.\n\n**① `position: absolute` 자식은 부모의 box 에 기여하지 않습니다.** render-prop 으로 임의의 자식을 받는 컴포넌트가 wrapper 의 `getBoundingClientRect()` 를 신뢰하면 자식이 absolute 일 때 silently 깨집니다.\n\n**② 측정 대상은 wrapper 보다 자식 (또는 가장 가까운 visible descendant) 이 안전합니다.** 사용자가 어떤 CSS 로 자식을 배치하든 자식 자체는 viewport 좌표가 있고, wrapper 는 자식의 layout 에 영향받습니다.\n\n**③ 디버깅 팁** — popover 가 엉뚱한 곳에 뜨면, DevTools console 에서 `el.getBoundingClientRect()` 를 wrapper 와 자식 양쪽에 찍어 보세요. wrapper 가 0×0 으로 나오면 그게 답입니다.",
