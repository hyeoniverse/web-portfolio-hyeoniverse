...
const rawTroubleShootingItems: TroubleShootingItem[] = [
  ...
  },
  {
    ...
    cause: {
      ko: "번역 API 는 DeepL · Google · Gemini · Claude 등 여러 곳을 골라 쓸 수 있도록 만들어 뒀습니다 (이런 외부 API 제공처를 \"provider\" 라고 부릅니다).\n\n그런데 \"여러 곳을 지원한다\" 는 게 \"실제로 그 여러 곳을 다 쓴다\" 는 뜻은 아니었습니다. 실제 동작은 \"이 기능엔 DeepL 만 쓴다\" 처럼 **한 번에 한 곳만 호출** 했고, 그게 실패하면 거기서 그냥 끝이었거든요. 다른 곳으로 자동으로 넘어가는 \"대체 경로 (fallback)\" 가 없었습니다.\n\n게다가 API 키가 등록되지 않은 provider 를 선택해 둔 상태에서도, UI 에서 \"지금 못 쓰는 기능\" 이라고 숨기거나 비활성화하지 않았습니다. 그래서 **사용자가 절대 성공할 수 없는 버튼을 계속 누를 수 있는** 상황이 됐죠.",
