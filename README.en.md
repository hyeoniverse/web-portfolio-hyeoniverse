...
| `revisions` | Editor revision history (shared for posts/works, JSONB snapshot, `entity_id text` — allows the new-post draft sentinel) |
...
| 81 | `npm i @vercel/analytics` (and `@vercel/speed-insights`) fails with ERESOLVE over a Svelte package that is never installed | Both packages declare vue, nuxt, svelte and friends as **optional peers** so it can ship per-framework entry points. Even though none of them get installed, npm 11 still walks their peers → `@sveltejs/kit` → `@sveltejs/vite-plugin-svelte@5` (peer `vite ^6`), which clashes with this repo's vite 8 (vitest, @vitejs/plugin-react) and the install aborts. `--legacy-peer-deps` would silence the whole check, including real conflicts such as the react 19.3 one that R3F blocks. Fix: add `"@sveltejs/vite-plugin-svelte": "^7"` (vite 8 support) to `overrides` so only npm's candidate changes. No svelte package ends up in the installed tree. **An optional peer is not "ignored when unused" — it still has to be resolvable** |
