...
- **Posts tag dropdown infinite scroll**: With tag counts hitting the hundreds, the dropdown now batches 40 at a time. A sentinel inside the dropdown is observed by `IntersectionObserver` and appends the next 40 when near the bottom. `tagRow` is capped at `max-height: 220px` with internal scroll so the dropdown can't blow up the filter bar height
...
| `works` | Portfolio works (team_members jsonb + `scheduled_at` for scheduled publishing + `purge_after` trash TTL) |
...
> 50+ issues encountered during development, with the top 18 surfaced on the About page (filtered by difficulty + generalizability via a `HIDDEN_PROBLEMS` Set — data is preserved and can be unhidden anytime). 6 sections (Architecture / Performance / Layout / Plate Editor / Animation·Interaction / Component) + difficulty (1–3) + recommended (★) badges. Highlights below — full list at **[docs/troubleshooting.en.md](./docs/troubleshooting.en.md)** or the About page.
