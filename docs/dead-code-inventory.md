...
`npm run knip` 결과의 baseline. [refactoring-guide.md](refactoring-guide.md) P7 — 추상화보다 삭제가 먼저다.

- **측정일**: 2026-07-23 / 커밋 `878560ab`
- knip 6.27.0

> knip 의 unused export 판정은 **동적 참조를 못 본다.** 문자열 키로 접근하거나 플러그인이 런타임에
> 집어가는 export 는 오탐이다. 아래 분류의 "보류"는 그 판단이 필요한 항목이다.
