...
| Styling | ![CSS Modules](https://img.shields.io/badge/CSS_Modules-1572b6?style=flat-square&logo=css3&logoColor=white) + ![Tailwind CSS](https://img.shields.io/badge/Tailwind_v4-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white) + CSS Variables |
...
**CHECK 제약**: `site_settings_about_erd_valid` — `site_settings.config` 에 `about_erd_valid(config)` 를 적용해 잘못된 ERD 설정이 저장되는 것을 DB 레벨에서 막는다 (앱의 `validateAboutErd` 검증이 뚫려도 최종 방어선). 자세한 근거는 [DB 설계 결정](./docs/db-design.md#about-studio-erd-설정-검증-about_erd_valid) 참고.
