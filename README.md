...
| Editor Blocks | ![React Flow](https://img.shields.io/badge/@xyflow/react-ff0072?style=flat-square) (에디터 다이어그램 · About ERD) ![Sandpack](https://img.shields.io/badge/Sandpack-000?style=flat-square) (React/TS 플레이그라운드) ![CodeMirror](https://img.shields.io/badge/CodeMirror_6-D30707?style=flat-square) (About ERD SQL 편집기 — 자동완성 · 찾기 · 진단) ![KaTeX](https://img.shields.io/badge/KaTeX-329894?style=flat-square) (수식) + Prettier (코드 포맷, 지연 로딩) |
...
| `revisions` | 에디터 리비전 히스토리 (posts/works 공용, JSONB snapshot, `entity_id text` — 새 글 draft sentinel 허용) |
...
**RPC 함수**: `sum_post_views()` (누적 조회수 합계), `daily_post_views(start, end)` (일별 시계열), `publish_scheduled()` (예약 시간 도달한 게시물/작품 발행 + 알림 + 이메일 — **pg_cron 매분**), `purge_trash_scheduled()` (`purge_after` 지난 휴지통 hard delete + 알림 — **pg_cron 매일 KST 03:00**), `about_erd_valid(config jsonb)` (About Studio 로 편집한 ERD 설정의 형태 검증 — 테이블/컬럼 이름 · 타입 비어있음 · 중복 차단 + 선택 필드 타입(`required`/`unique`/`indexed`/`defaultValue`/`comment`/`enumValues`/`kind`) 검사, IMMUTABLE), `settings_required_valid(config jsonb)` (사이트 설정 필수값 검증 — 제목 · 이름 · 테마 색상 5종 · 멤버 이름 비어있음 차단 + giscus 선택 시 repo 계열 필수 + 이메일 형식, IMMUTABLE)

**CHECK 제약**: `site_settings_about_erd_valid` — `site_settings.config` 에 `about_erd_valid(config)` 를 적용해 잘못된 ERD 설정이 저장되는 것을 DB 레벨에서 막는다 (앱의 `validateAboutErd` 검증이 뚫려도 최종 방어선). `site_settings_required_valid` — 같은 `site_settings.config` 에 `settings_required_valid(config)` 를 적용해 필수값(제목·이름·테마색·giscus·멤버이름)이 빈 채로 저장되는 것을 막는다. 자세한 근거는 [DB 설계 결정](./docs/db-design.md#about-studio-erd-설정-검증-about_erd_valid) 참고.
...
| 81 | `npm i @vercel/analytics`(·`@vercel/speed-insights`) 가 설치도 안 되는 Svelte 패키지 때문에 ERESOLVE | 두 패키지 모두 프레임워크별 진입점을 위해 vue·nuxt·svelte 등을 **optional peer** 로 선언한다. 설치되지 않는 의존성인데도 npm 11 은 그 후보들의 peer 까지 따라가 검증한다 → `@sveltejs/kit` → `@sveltejs/vite-plugin-svelte@5`(peer `vite ^6`)가 이 저장소의 vite 8(vitest·@vitejs/plugin-react)과 충돌해 설치 자체가 실패. `--legacy-peer-deps` 는 이 검사 전체를 끄는 것이라 다른 진짜 충돌(R3F 가 막아 둔 react 19.3 같은)까지 못 잡는다. 해결: `overrides` 에 `"@sveltejs/vite-plugin-svelte": "^7"`(vite 8 지원) 을 넣어 npm 이 보는 후보만 바꾼다. 실제 설치 트리에는 svelte 관련 패키지가 하나도 들어오지 않는다. **optional peer 는 "안 쓰면 무시" 가 아니라 "해결 가능해야 통과" 다** |
