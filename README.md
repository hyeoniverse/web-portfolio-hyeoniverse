...
| Editor Blocks | ![React Flow](https://img.shields.io/badge/@xyflow/react-ff0072?style=flat-square) (에디터 다이어그램 · About ERD) ![Sandpack](https://img.shields.io/badge/Sandpack-000?style=flat-square) (React/TS 플레이그라운드) ![KaTeX](https://img.shields.io/badge/KaTeX-329894?style=flat-square) (수식) + Prettier (코드 포맷, 지연 로딩) |
...
**RPC 함수**: `sum_post_views()` (누적 조회수 합계), `daily_post_views(start, end)` (일별 시계열), `publish_scheduled()` (예약 시간 도달한 게시물/작품 발행 + 알림 + 이메일 — **pg_cron 매분**), `purge_trash_scheduled()` (`purge_after` 지난 휴지통 hard delete + 알림 — **pg_cron 매일 KST 03:00**), `about_erd_valid(config jsonb)` (About Studio 로 편집한 ERD 설정의 형태 검증 — 테이블/컬럼 이름 · 타입 비어있음 · 중복 차단, IMMUTABLE)
